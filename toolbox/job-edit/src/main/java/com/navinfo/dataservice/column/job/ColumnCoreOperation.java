...
public class ColumnCoreOperation {
	 ...
	 * 实现逻辑
	 *  1)	取数据检查结果
	 *	2)	检查结果和poi_column_status里已记录的work_item_id做差分比较
	 *	3)	若同一一级项或者二级项下，①已记录了该检查项，则不做处理；②没记录则新增一条记录；③若检查结果没有该规则号，classifyRules中有，poi_column_status表中记录了该规则号，则从poi_column_status表删除该记录
	 * @param mapParams
	 * @throws Exception
	 */
	...
	public void insertWorkItem(List<String> checkResultList,Connection conn,int pid) throws Exception {
		...
		sb.append("insert into poi_column_status(pid,work_item_id,first_work_status,second_work_status,handler) values(?,?,?,?,?) ");
