 ...
 * @Description TODO
 *              1）一根link上的道路名（包括官方名和别名）如果只有1种类型且为虚拟名称或线门牌时，名称只能有一个为官方名，其他全部为别名，
 *              否则报Log1； 2）如果名称类型有多种时，虚拟名称或线门牌只能为别名，否则报Log2
 *              3）虚拟名称或线门牌类型的道路名不能为曾用名，否则报Log3 4）如果名称类型中既有虚拟名称又有线门牌时，报Log4
 *              名称分类编辑 服务端后检查 名称类型编辑 服务端后检查
 */
public class GLM02012 extends baseRule {
	...
	private List<Object> check(int pid) throws Exception {
		...
		sb.append("SELECT 0 AS GEOMETRY, '[RD_LINK,' || LN.LINK_PID || ']' TARGET,0 AS MESH_ID,");
		...
		sb.append(" UNION");
		sb.append(" SELECT 0 AS GEOMETRY, '[RD_LINK,' || LN1.LINK_PID || ']' TARGET,0 AS MESH_ID,");
