...
public class FillLeftDeleteLogDetail implements ResultSetHandler<String> {
	...
	public FillLeftDeleteLogDetail(GlmTable table,OracleSchema diffServer){
		this.table=table;
...
//		List<ColumnMetaData> tmdList = DataBaseUtils.getTableMetaData(table.getName(), mData);
		String updateSql = "UPDATE LOG_DETAIL SET \"OLD\"=? WHERE TB_ROW_ID=HEXTORAW(?) AND TB_NM='"+table.getName()+"'";
