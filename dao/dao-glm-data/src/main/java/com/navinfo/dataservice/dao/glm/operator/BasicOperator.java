...
public class BasicOperator extends AbstractOperator {
        ...
        for (Field f : fields) {
            if (isBasicType(f)) {
                ...
                if (oj instanceof Geometry) {
                    ...
                    if (wkt.length() > 4000) {
                        key.insert(0, "DECLARE v_geo SDO_GEOMETRY := sdo_geometry('" + wkt + "', 8307); BEGIN ");
                        oj = "v_geo";
                        complexGeo = true;
                    } else {
            ...
            if (value instanceof String || value == null) {
                ...
                if (!StringUtils.isStringSame(oldWkt, newWkt)) {
                    if (newWkt.length() > 4000) {
                        sb.insert(0, "DECLARE v_geo SDO_GEOMETRY := sdo_geometry('" + newWkt.trim() + "',8307); BEGIN ");
    ...
    public String getWhereForTable(IRow row) {
        try {
            if (row.getClass().getMethod(M_PRIMARYKEY).invoke(row) != null) {
                return "" + row.getClass().getMethod("M_PRIMARYKEY").invoke(row) + " = " + row.getClass().getMethod
                        (M_PID).invoke(row) + "";
            } else {
    ...
    private void addConditionForPoi(StringBuilder sb) throws SQLException {
        String tableName = SelectorUtils.getObjTableName(row);
        if (tableName.equals("IX_POI") || row instanceof IxPoi || row instanceof IxPoiChildren || row instanceof
                IxPoiParent) {
