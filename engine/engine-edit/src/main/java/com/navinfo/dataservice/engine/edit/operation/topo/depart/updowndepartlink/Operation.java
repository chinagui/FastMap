...
public class Operation implements IOperation {
    ...
    private boolean checkLineConnect(LineString[] lineStrings) throws Exception {
        ...
        for (int i = 0; i < lineStrings.length - 1; i++) {
            ...
            if (!(preS.equals(nextS) || preS.equals(nextN) || preN.equals(nextS) || preN.equals(nextN))) {
                ...
                String errInfo = "所选第 " + String.valueOf(i + 1) + "条link与第 " + String.valueOf(i + 2) +
    ...
    private void createDepartLinks(Result result) throws Exception {
        ...
        for (int i = 0; i < links.size(); i++) {
            lineStrings[i] = (JtsGeometryFactory.createLineString(GeoTranslator.transform(links.get(i).getGeometry(),
                    0.00001, 5).getCoordinates()));
        }
        ...
        // 传入起点和终点Point
        Point sPoint = JtsGeometryFactory.createPoint(GeoTranslator.transform(this.getStartAndEndNode(links, 0)
                .getGeometry(), 0.00001, 5).getCoordinate());
    ...
    private void createInnerLines(LineString[] lines, Map<Geometry, RdNode> map, Result result) throws Exception {
        ...
        for (int i = 0; i < command.getLinkPids().size() - 1; i++) {
            ...
            List<RdLink> links = nodeSelector.loadByDepartNodePid(currentPid, currentLink.getPid(), nextLink.getPid()
                    , true);
            ...
            if (flagBooleans.size() > 1) {
                // 计算生成link的方向
                int driect = this.createInnerLinkDir(currentPid, rightLinks, leftLinks);
                this.createInnerLine(lines[i], lines[lines.length - i - 1], map, driect, result);
     ...
     * @param leftLinks
     */
    private int createInnerLinkDir(int currentPid, List<RdLink> rightLinks, List<RdLink> leftLinks) {
        // 默认双方向
        int direct = 1;
        for (RdLink rLink : rightLinks) {
            ...
            for (RdLink lLink : leftLinks) {
                ...
                if (((rLink.getDirect() == 2 && rLink.geteNodePid() == currentPid) || (rLink.getDirect() == 3 &&
                        rLink.getsNodePid() == currentPid)) && ((lLink.getDirect() == 2 && lLink.geteNodePid() ==
                        currentPid) || (lLink.getDirect() == 3 && lLink.getsNodePid() == currentPid))) {
                    return 1;
                }
                if (((rLink.getDirect() == 2 && rLink.getsNodePid() == currentPid) || (rLink.getDirect() == 3 &&
                        rLink.geteNodePid() == currentPid)) && ((lLink.getDirect() == 2 && lLink.getsNodePid() ==
                        currentPid) || (lLink.getDirect() == 3 && lLink.geteNodePid() == currentPid))) {
                    return 1;
                }

            }
            RdLink flagLink = rightLinks.get(0);
            if ((flagLink.getDirect() == 2 && flagLink.getsNodePid() == currentPid) || (flagLink.getDirect() == 3 &&
                    flagLink.geteNodePid() == currentPid)) {
                direct = 3;
            } else {
                direct = 2;
            }
    ...
    private void createInnerLine(LineString lineDownString, LineString lineUpString, Map<Geometry, RdNode> map, int
            direct, Result result) throws Exception {
        ...
        for (int i = 0; i < command.getLinkPids().size() - 1; i++) {
            ...
            for (RdLink link : links) {
                ...
                if (this.isRightSide(currentLink, nextLink, link)) {
                    targetLine = CompPolylineUtil.cut(lines[i], lines[i + 1], JtsGeometryFactory.createLineString
                            (GeoTranslator.transform(link.getGeometry(), 0.00001, 5).getCoordinates()), currentPoint,
                            true);
                } else {
                    targetLine = CompPolylineUtil.cut(lines[lines.length - i - 1], lines[lines.length - i - 2],
                            JtsGeometryFactory.createLineString(GeoTranslator.transform(link.getGeometry(), 0.00001,
                                    5).getCoordinates()), currentPoint, false);
                }
     ...
     * @throws Exception
     */
    private void updateadjacentLine(LineString targetLine, RdLink link, int currentPid, Map<Geometry, RdNode> map,
    ...
    private void relationLimitForLink(RdLink link) {
        ...
        for (IRow row : link.getLimits()) {
            ...
            if (limit.getType() == 1 || limit.getType() == 2 || limit.getType() == 3 || limit.getType() == 5 || limit
                    .getType() == 6 || limit.getType() == 7) {
     ...
     * @throws Exception
     */
    private List<RdLink> createUpRdLink(RdLink departLink, Result result, RdLink sourceLink, RdLink sourceNextLink,
                                        Map<Geometry, RdNode> map) throws Exception {
        ...
        List<RdLink> links = linkSelector.loadByDepartNodePid(intersectPid, sourceLink.getPid(), sourceNextLink
                .getPid(), true);
        // 如果对应起点没有挂接的link
        // 对于上(左)线需要生成新的node
        if (links.size() <= 0) {
     ...
     * @throws Exception
     */
    private List<RdLink> createDownRdLink(RdLink departLink, Result result, RdLink sourceLink, RdLink sourceNextLink,
                                          Map<Geometry, RdNode> map) throws Exception {
        RdLinkSelector linkSelector = new RdLinkSelector(conn);
     ...
     * @throws Exception
     */
    private List<RdLink> createEndRdLink(RdLink departLink, Result result, RdLink sourceLink, RdLink sourceNextLink,
                                         Map<Geometry, RdNode> map) throws Exception {
        RdNode sNode = null;
        RdNode eNode = null;
        sNode = map.get(JtsGeometryFactory.createPoint(departLink.getGeometry().getCoordinates()[0]));
        eNode = map.get(JtsGeometryFactory.createPoint(departLink.getGeometry().getCoordinates()[departLink
     ...
     * @throws Exception
     */
    private RdNode getDepartRdlinkNode(List<RdLink> links, RdLink departLink, RdLink sourceLink, RdLink
            sourceNextLink, int flag, int flagUpDown, Map<Geometry, RdNode> map, Result result) throws Exception {
        ...
        if (flag == 1) {
            if (flagUpDown == 0) {
                if (sourceLink.getsNodePid() == command.getsNode().getPid() || sourceLink.geteNodePid() == command
                        .getsNode().getPid()) {
                    return command.getsNode();
                }
            }
            if (flagUpDown == 1) {
                if (sourceLink.getsNodePid() == command.geteNode().getPid() || sourceLink.geteNodePid() == command
                        .geteNode().getPid()) {
     ...
     * @throws Exception
     */
    private RdNode getNodeByDepartGeo(RdLink departLink, int flag, Map<Geometry, RdNode> map, Result result) throws
            Exception {
        RdNode node = null;
        ...
        if (flag == 1) {
            ...
            coordinate = departLink.getGeometry().getCoordinates()[departLink.getGeometry().getCoordinates().length -
                    1];
        }
    ...
    private RdNode updateAdNodeForTrack(RdLink link, int nodePid, Map<Geometry, RdNode> map, Result result, int flag)
            throws Exception {
        JSONObject updateContent = new JSONObject();
        ...
        if (flag == 1) {
            ...
            geometry = JtsGeometryFactory.createPoint(link.getGeometry().getCoordinates()[link.getGeometry()
