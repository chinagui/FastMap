...
public class CheckEngine {
	...
	public CheckEngine(CheckCommand checkCommand) throws Exception {
		...
		// this.conn =
		// GlmDbPoolManager.getInstance().getConnection(this.checkCommand.getProjectId());
		// this.conn.setAutoCommit(true);
	}

	public CheckEngine(CheckCommand checkCommand, Connection conn, int projectId)
			throws Exception {
		this.log = LoggerRepos.getLogger(this.log);
		this.checkCommand = checkCommand;
		this.conn = conn;
	 ...
	 * 获取本次要执行的检查规则
	 */
	private List<CheckRule> getRules(ObjType objType, OperType operType,
			String checkType) throws Exception {
		String suitCode = objType.toString() + "_" + operType.toString() + "_"
				+ checkType;
		log.info(suitCode);
		List<CheckRule> myCheckSuit = CheckSuitLoader.getInstance()
				.getCheckSuit(suitCode);
		this.myCheckSuitVariables = CheckSuitLoader.getInstance()
				.getCheckSuitVariables(suitCode);
		return myCheckSuit;
	}

	private List<CheckRule> getRulesTest(ObjType objType, OperType operType,
			String checkType) throws Exception {
		...
		CheckRule rule = new CheckRule(initRuleCode, initRuleLog, initSeverity,
				initCheckClassPath, accessorType, accessorName, variables);

		String ruleCode = "GLM01197";
		String ruleLog = "log";
		String ruleClass = "com.navinfo.dataservice.engine.check.rules.GLM01197";

		CheckRule rule2 = new CheckRule(ruleCode, ruleLog, 1, ruleClass, null,
				null, null);
		CheckRule rule3 = new CheckRule(ruleCode, ruleLog, 1, ruleClass, null,
				null, null);
	 ...
	 */
	private void saveCheckResult(List<NiValException> checkResultList)
			throws Exception {
		...
		for (int i = 0; i < checkResultList.size(); i++) {
			...
			check.insertCheckLog(checkResultList.get(i).getRuleId(),
					checkResultList.get(i).getLoc(), checkResultList.get(i)
							.getTargets(), checkResultList.get(i).getMeshId(),
					checkResultList.get(i).getInformation(), "TEST");
		}
	}
	...
	public String preCheck() throws Exception {
		...
		// isValidConn();
		// 获取前检查需要执行规则列表
		List<CheckRule> rulesList = getRules(checkCommand.getObjType(),
				checkCommand.getOperType(), "PRE");
	...
	public void postCheck() throws Exception {
		...
		// isValidConn();
		// 获取后检查需要执行规则列表
		List<CheckRule> rulesList = getRules(this.checkCommand.getObjType(),
				this.checkCommand.getOperType(), "POST");
		List<NiValException> checkResultList = new ArrayList<NiValException>();
		RuleExecuter ruleExecuterObj = new RuleExecuter(this.checkCommand,
				this.myCheckSuitVariables, this.conn);
		for (int i = 0; i < rulesList.size(); i++) {
			CheckRule rule = rulesList.get(i);
			try {
				List<NiValException> resultTmp = ruleExecuterObj.exeRule(rule);
				if (resultTmp.size() > 0) {
					checkResultList.addAll(resultTmp);
				}
			} catch (Exception e) {
