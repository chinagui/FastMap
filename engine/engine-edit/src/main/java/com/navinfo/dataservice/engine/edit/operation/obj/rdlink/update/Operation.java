...
public class Operation implements IOperation {
	...
	public String run(Result result) throws Exception {
		...
		// 批量修改
			...
			for (int i = 0; i < command.getLinks().size(); i++) {
				...
				command.setUpdateContent(command.getUpdateContents()
						.getJSONObject(i));
				updateLink(result);
			}
		}
	...
	private void saveForms(Result result, JSONArray forms) throws Exception {
		...
		for (int i = 0; i < forms.size(); i++) {
			...
			if (formJson.containsKey("objStatus")) {
				...
				if (insertCount == 0
						&& deleteCount == updateLink.getForms().size()) {
	 ...
	 * @throws Exception
	 */
	private void saveTmcLocations(Result result, JSONArray array)
			throws Exception {
		for (int i = 0; i < array.size(); i++) {
			JSONObject json = array.getJSONObject(i);
			RdTmclocation obj = updateLink.locationMap.get(json
					.getString("rowId"));
			if (json.containsKey("objStatus")) {
				if (ObjStatus.DELETE.toString().equals(
						json.getString("objStatus"))) {
	 ...
	 * @throws Exception
	 */
	private void saveTmcLocationLinks(Result result, JSONArray array,
			RdTmclocation obj) throws Exception {
		for (int i = 0; i < array.size(); i++) {
			JSONObject json = array.getJSONObject(i);
	...
	private void refRdLaneForRdLinkLaneNum(Result result) throws Exception {
		...
		int laneLeft = 0;
		int laneRight = 0;
		if (content.containsKey("laneNum") || content.containsKey("laneLeft")
				|| content.containsKey("laneRight")) {
	...
	public String updateRdTraffic(Result result) throws Exception {
		...
		if (changeFields.containsKey("direct")) {
			com.navinfo.dataservice.engine.edit.operation.obj.trafficsignal.update.Operation operation = new com.navinfo.dataservice.engine.edit.operation.obj.trafficsignal.update.Operation(
					conn);
			List<RdTrafficsignal> trafficsignalList = operation
					.updateRdCrossByModifyLinkDirect(updateLink);
	 ...
	 * @throws Exception
	 */
	public List<AlertObject> getUpdateRdLinkAlertData(RdLink updateLink,
