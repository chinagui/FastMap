...
/**
 * 	FM-D01-43		POI简称标准中文名称全部为符号检查			DHM	
	检查条件：
	 	非删除POI对象 
	检查原则：
	标准化中文名称中，除了全角合法特殊字符（通用元数据库ty_character_egalchar_ext中extention_type 为SYMBOL_F 的记录）外，没有其他字符即全是字符，则报log：名称全部是字符，需要删除设施！
	name_type=1，name_class=5，lang_code=CHI或CHT
 * @author sunjiawei
 *
 */
public class FMD0143 extends BasicCheckRule {
		...
		if(obj.objName().equals(ObjectName.IX_POI)){
			...
			for(IxPoiName nameTmp:names){
				if(nameTmp.isCH()&&nameTmp.getNameType()==1
					...
					Map map  = metadataApi.tyCharacterEgalcharExtGetExtentionTypeMap();
