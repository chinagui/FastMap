 ...
 * @ClassName GLM03072
 * @author Han Shaoming
 * @date 2017年3月23日 上午9:41:16
 * @Description TODO
 * 点属性为“图廓点”时，种别必须是“平面交叉点”
 *  node属性编辑	服务端后检查
 * node种别编辑	服务端后检查
 */
public class GLM03072 extends baseRule {
	...
	private void checkRdNode(RdNode rdNode) throws Exception {
		...
		if(checkFlag){
			...
			sb.append(" AND F.FORM_OF_WAY = 2 AND F.U_RECORD != 2");
			String sql = sb.toString();
			log.info("RdNode后检查GLM03072--sql:" + sql);
	...
	private void checkRdNodeForm(RdNodeForm rdNodeForm) throws Exception {
		...
		if(checkFlag){
			...
			sb.append(" AND N.KIND <> 1 AND N.U_RECORD != 2");
			String sql = sb.toString();
			log.info("RdNode后检查GLM03072--sql:" + sql);
