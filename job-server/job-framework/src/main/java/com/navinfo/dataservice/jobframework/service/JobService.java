...
public class JobService {
	...
	public JSONObject getLatestJob(int subtaskId)throws ServiceException{
		...
		try{
			...
			conn = MultiDataSourceFactory.getInstance().getSysDataSource()
					.getConnection();
			String jobInfoSql = "select j.job_id , j.job_guid  from job_info j where j.job_type = 'checkCore' and j.descp = '元数据库检查' and j.task_id = "+subtaskId+" and j.end_time is not null and rownum <=1 order by j.end_time desc ";
