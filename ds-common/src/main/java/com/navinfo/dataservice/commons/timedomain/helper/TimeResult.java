...
public String getTimeSpanResult()
{
	...
	for(int i=0; i < m_vTimeSpan.size(); i++){
		...
		Set<Map.Entry<String, Pair<Integer, Integer>>> set=m_vTimeSpan.get(i).entrySet();
		
		Iterator<Map.Entry<String, Pair<Integer, Integer>>> iter=set.iterator();
...
public String getCHITimeResult()
{
	...
	if(sStartDay.isEmpty() && sEndDay.isEmpty()){
		if(sStartTimeYearMonth.isEmpty() && sEndTimeYearMonth.isEmpty()){
			if(sStartWeek.isEmpty() && sEndWeek.isEmpty()){
				...
				if(sStartWeek == sEndWeek){
					...
					sOut += sStartWeek + "到" + sEndWeek + " " + sStartHour + "到"+ sEndHour;
			}
		}else{
			if(sStartTimeYearMonth != sEndTimeYearMonth){
				if(sStartWeek != sEndWeek){
					sOut += sStartTimeYearMonth + "到" + sEndTimeYearMonth + sStartWeek + "到" + sEndWeek + " " + sStartHour + "到"+ sEndHour;
				}else{
					sOut += sStartTimeYearMonth + "到" + sEndTimeYearMonth + sStartWeek +  " " + sStartHour + "到"+ sEndHour;
				}
			}else{
				if(sStartWeek != sEndWeek){
					sOut += sEndTimeYearMonth +  sStartWeek + "到" + sEndWeek + " " + sStartHour + "到"+ sEndHour;
				}else{
					sOut += sEndTimeYearMonth + sStartWeek + " " + sStartHour + "到"+ sEndHour;
				}
			}
		}
	}else{
		...
		if(nIdex != -1){
			...
			if(cIn== ')'){
				sOut += sStartTimeYearMonth + sStartDay + "日" + "到" + sEndTimeYearMonth + sEndDay + "日" + ", " + sStartHour + "到" + sEndHour;
			}else{
				sOut += sStartTimeYearMonth + sStartDay + "日" + sStartHour + "到" + 
						sEndTimeYearMonth + sEndDay + "日" + sEndHour;
			}
		}
	}
