...
import com.navinfo.dataservice.control.service.PaService;
...
public class PaController extends BaseController {
	...
	@RequestMapping(value = "/pa/upload")
	public ModelAndView paUpload(HttpServletRequest request){
		int jobId = 0;
		int subtaskId = 0;
		long userId = 0;
		...
		try{
			...
			result = PaService.getInstance().upload(jobId, subtaskId, userId);
			...
			//记录上传日志。不抛出异常
            insertStatisticsInfoNoException(jobId, subtaskId, userId,
            		result,beginTime,"pa 点门牌上传数据失败:"+e.getMessage());
