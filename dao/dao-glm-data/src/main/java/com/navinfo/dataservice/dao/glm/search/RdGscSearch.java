...
public class RdGscSearch implements ISearch {
	...
	public List<SearchSnapshot> searchDataByTileWithGap(int x, int y, int z,
		...
		String sql = "WITH TMP1 AS (SELECT A.PID FROM RD_GSC A WHERE A.U_RECORD != 2 AND SDO_WITHIN_DISTANCE(A.GEOMETRY, SDO_GEOMETRY(:1, 8307), 'DISTANCE=0') = 'TRUE'), TMP2 AS (SELECT A.PID FROM RD_GSC_LINK A, TMP1 B WHERE A.PID = B.PID AND A.U_RECORD != 2 AND A.TABLE_NAME IN ('RD_LINK', 'RW_LINK') GROUP BY A.PID HAVING COUNT(1) > 1), TMP3 AS (SELECT A.*, B.GEOMETRY FROM RD_GSC_LINK A, RD_LINK B, TMP2 C WHERE A.LINK_PID = B.LINK_PID AND A.U_RECORD != 2 AND A.TABLE_NAME = 'RD_LINK' AND A.PID = C.PID), TMP4 AS (SELECT A.*, B.GEOMETRY FROM RD_GSC_LINK A, RW_LINK B, TMP2 C WHERE A.LINK_PID = B.LINK_PID AND A.U_RECORD != 2 AND A.TABLE_NAME = 'RW_LINK' AND A.PID = C.PID) SELECT TMP.*, A.GEOMETRY AS GSC_GEO FROM (SELECT * FROM TMP3 UNION ALL SELECT * FROM TMP4) TMP, RD_GSC A WHERE TMP.PID = A.PID ORDER BY TMP.PID, TMP.ZLEVEL ";
