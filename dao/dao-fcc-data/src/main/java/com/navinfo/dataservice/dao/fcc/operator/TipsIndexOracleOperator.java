...
public class TipsIndexOracleOperator implements TipsIndexOperator {
	...
	private static String insertSql = "INSERT INTO TIPS_INDEX(ID,STAGE,T_DATE,T_OPERATEDATE,T_LIFECYCLE,HANDLER,S_SOURCETYPE,WKT,S_QTASKID,S_MTASKID,S_QSUBTASKID,S_MSUBTASKID,WKTLOCATION,T_TIPSTATUS,T_DEDITSTATUS,T_MEDITSTATUS,S_PROJECT,T_DEDITMETH,T_MEDITMETH,T_DATADATE) VALUES (?,?,TO_TIMESTAMP(?,'yyyyMMddHH24miss'),TO_TIMESTAMP(?,'yyyyMMddHH24miss'),?,?,?,SDO_GEOMETRY(?,8307),?,?,?,?,SDO_GEOMETRY(?,8307),?,?,?,?,?,?,TO_TIMESTAMP(?,'yyyyMMddHH24miss'))";
	...
	private List<TipsDao> loadHbaseProperties(Map<String, TipsDao> map) throws Exception{
    	List<TipsDao> result = new ArrayList<>();
		String[] queryColNames = { "deep", "geometry", "feedback", "tipdiff", "old" };
