...
public class Operation implements IOperation {
	...
	private void caleCatchs(Result result) throws Exception {
		if (this.command.getCatchInfos() != null
			...
			for (int i = 0; i < this.command.getCatchInfos().size(); i++) {
				...
				AdNode preNode = (AdNode) nodeSelector.loadById(nodePid, true,
						true);
				// 分离node挂接的link
	...
	private void departCatchtNode(Result result, int nodePid, int catchNodePid,
			AdNode preNode, List<AdLink> links) throws Exception {
		AdNodeSelector nodeSelector = new AdNodeSelector(conn);
		// 用分离挂接的Node替换修行Link对应的几何,以保持精度
		AdNode catchNode = (AdNode) nodeSelector.loadById(catchNodePid, true,
				true);
		// 获取挂接Node的几乎额
	 ...
	 * @throws Exception
	 */
	private void departCatchBreakLink(double lon, double lat, AdNode preNode,
			int linkPid, List<AdLink> links, Result result) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void moveNodeGeo(AdNode node, double lon, double lat, Result result)
			throws Exception {
				...
				if (geomInter instanceof GeometryCollection) {
					...
					for (int i = 0; i < geoNum; i++) {
						...
						if (subGeo instanceof LineString) {
							...
							links.addAll(AdLinkOperateUtils
									.getCreateAdLinksWithMesh(subGeo, maps,
											result,
