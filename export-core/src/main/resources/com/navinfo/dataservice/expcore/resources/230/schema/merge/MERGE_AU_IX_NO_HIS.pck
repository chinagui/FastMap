CREATE OR REPLACE PACKAGE merge_au_ix_no_his IS
  ...
  PROCEDURE reset_tmp_ix_poi(v_merge_type VARCHAR2);
...
CREATE OR REPLACE PACKAGE BODY merge_au_ix_no_his IS
  ...
  BEGIN
    DELETE FROM temp_his_ix_poi_name;
    INSERT INTO temp_his_ix_poi_name
      SELECT * FROM ix_poi_name;
  END;
  PROCEDURE reset_tmp_ix_poi(v_merge_type VARCHAR2) IS
     v_oprstatus_clause VARCHAR2(100);
  BEGIN
    /*v_oprstatus_clause := merge_utils.get_oprstatus_clause(v_merge_type,
                                                           'au');
    --为了记录变化前履历，临时保存POI
    DELETE FROM temp_his_ix_poi;
    execute immediate 'INSERT INTO temp_his_ix_poi
      SELECT * FROM ix_poi t where t.pid in(select pid from au_ix_poi au where  '||v_oprstatus_clause||')';
      */
    DELETE FROM temp_his_ix_poi;
    INSERT INTO temp_his_ix_poi
      SELECT * FROM ix_poi t ;
  END;
  ...
  procedure reset_tmp_ix_buiding is
    begin
       DELETE FROM temp_his_ix_poi_building;
 ...
 (SELECT 1
  ...
  SELECT au.pid, ''110000030000'', 0 AS u_record, NULL AS u_fields
   ...
   WHERE au.state = '||v_state||'
	...
	AND EXISTS (SELECT 1
            FROM au_ix_poi_flag aipf
           WHERE aipf.audata_id = au.audata_id
             AND aipf.flag_code = ''110000030000'')';*/
  ...
  procedure mul_add_tenant(v_audata_id NUMBER) is
    ...
    /*   execute immediate '
     ...
     WHERE EXISTS (SELECT 1
              FROM au_ix_poi_parent au, au_ix_poi auip
             WHERE au.audata_id = auip.audata_id
               AND auip.att_oprstatus = 0
               AND auip.state = 3
               AND p.parent_poi_pid = au.parent_poi_pid
     ...
     delete from IX_POI_FLAG ipf -- 删除 IX_POI_FLAG 中记录
      where ipf.poi_pid in
            (select c1.poi_pid
              ...
              WHERE c1.contact_type in (1,2) 
                and EXISTS (SELECT 1
                       FROM temp_au_poi_modify_log l
                      WHERE l.tel_flag = 1
                        AND c1.poi_pid = l.pid)
                AND NOT EXISTS (SELECT 1
                       FROM au_ix_poi_contact auipc,temp_au_poi_modify_log l
                      WHERE auipc.audata_id=l.audata_id                    
                        AND auipc.poi_pid = c1.poi_pid
  ...
  procedure process_modify_poi_name2 is
    ...
    begin
       delete from temp_ix_poi_name_mg;
       --通过POI_PID判断IX_POI_NAME表中Lang_Code为”CHI”、 NAME_CLASS为官方名称（1），名称类型（NAME_TYPE）为标准（2）的记录是否存在，如果不存在，则增加一条记录；否则修改该记录的NAME和NAME_PHONETIC的值。同时更新IX_POI的old_name字段
  ...
  BEGIN
    ...
    :1	修改整条中文地址记录的数据；
    :2	删除对应英文和葡文地址记录；
    :3	同时通过POI_PID删除IX_POI_FLAG表中FLAG_CODE 为以下值的记录，如果没有，则不处理；
 ...
 END;
  ...
  BEGIN
    ...
    INSERT INTO  TEMP_IX_POI_RESTAURANT_MG   
     SELECT * FROM IX_POI_RESTAURANT A WHERE EXISTS (
                   SELECT 1 FROM  temp_au_poi_modify_log l WHERE
                   L.PID = A.POI_PID AND l.food_type_flag = 1
     );
  ...
  BEGIN
    ...
    --改RELATION: 如果LOG字段为“改RELATION”，则需要通过POI_PID修改IX_POI表中GEOMETRY 、Mesh_ID、Pmesh_ID字段值。
    --改位移:X_GUIDE、Y_GUIDE
    --改地址:OLD_ADDRESS
  ...
  BEGIN
    ...
    END IF;
    v_log := upper(rec.log);
    IF rec.att_oprstatus = 0 AND instr(v_log, '改名称') > 0 THEN
      v_rec.name_flag := 1;
      v_change_flag   := TRUE;
    ELSE
      v_rec.name_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改地址') > 0 THEN
      v_rec.address_flag := 1;
      v_change_flag      := TRUE;
    ELSE
      v_rec.address_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改电话') > 0 THEN
      v_rec.tel_flag := 1;
      v_change_flag  := TRUE;
    ELSE
      v_rec.tel_flag := 0;
    END IF;   
    IF rec.att_oprstatus = 0 AND instr(v_log, '改分类') > 0 THEN
      v_rec.kind_flag := 1;
      v_change_flag   := TRUE;
    ELSE
      v_rec.kind_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改邮编') > 0 THEN
      v_rec.post_code_flag := 1;
      v_change_flag        := TRUE;
    ELSE
      v_rec.post_code_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改多义性') > 0 THEN
      v_rec.food_type_flag := 1;
      v_change_flag        := TRUE;
    ELSE
      v_rec.food_type_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改FATHERSON') > 0 THEN
      v_rec.parent_flag := 1;
      v_change_flag     := TRUE;
    ELSE
      v_rec.parent_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改标注') > 0 THEN
      v_rec.lable_flag := 1;
      v_change_flag    := TRUE;
    ELSE
      v_rec.lable_flag := 0;
    END IF;
    IF rec.geo_oprstatus = 0 AND instr(v_log, '改RELATION') > 0 THEN
      v_rec.display_point_flag := 1;
      v_change_flag            := TRUE;
    ELSE
      v_rec.display_point_flag := 0;
    END IF;
    IF rec.geo_oprstatus = 0 AND instr(v_log, '改位移') > 0 THEN
      v_rec.guide_point_flag := 1;
      v_change_flag          := TRUE;
    ELSE
      v_rec.guide_point_flag := 0;
    END IF;
    IF rec.geo_oprstatus = 0 AND instr(v_log, '改GUIDEX') > 0 THEN
      v_rec.guide_x_flag := 1;
      v_change_flag      := TRUE;
    ELSE
      v_rec.guide_x_flag := 0;
    END IF;
    IF rec.geo_oprstatus = 0 AND instr(v_log, '改GUIDEY') > 0 THEN
      v_rec.guide_y_flag := 1;
      v_change_flag      := TRUE;
    ELSE
      v_rec.guide_y_flag := 0;
    END IF;
    IF (rec.att_oprstatus = 0 AND v_rec.kind_flag = 1) THEN
      v_rec.chain_flag := 1;
      v_change_flag    := TRUE;
    ELSE
      v_rec.chain_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 and instr(v_log, '改TENANT_FLAG') > 0 THEN
      v_rec.tenant_flag := 1;
    ...
    ELSE
      v_rec.tenant_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 and instr(v_log, '改FLOOR_USED') > 0 THEN
      v_rec.FLOOR_USED := 1;
      v_change_flag    := TRUE;
    ELSE
      v_rec.FLOOR_USED := 0;
    END IF;
    IF rec.att_oprstatus = 0 and instr(v_log, '改FLOOR_EMPTY') > 0 THEN
      v_rec.FLOOR_EMPTY := 1;
      v_change_flag    := TRUE;
    ELSE
      v_rec.FLOOR_EMPTY := 0;
    END IF;
    IF rec.att_oprstatus = 0 and instr(v_log, '改POI_LEVEL') > 0 THEN
      v_rec.poilevel_flag := 1;
      v_change_flag    := TRUE;
    ELSE
      v_rec.poilevel_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 and instr(v_log, '改外业验证标识') > 0 THEN
      v_rec.verified_flag := 1;
      v_change_flag    := TRUE;
    ELSE
      v_rec.verified_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改24小时') > 0 THEN
      v_rec.open24h_flag := 1;
      v_change_flag  := TRUE;
    ELSE
      v_rec.open24h_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改内部POI') > 0 THEN
      v_rec.innerpoi_flag := 1;
      v_change_flag  := TRUE;
    ELSE
      v_rec.innerpoi_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改同一关系') > 0 THEN
      v_rec.samepoi_flag := 1;
      v_change_flag  := TRUE;
    ELSE
      v_rec.samepoi_flag := 0;
    END IF;
    IF rec.att_oprstatus = 0 AND instr(v_log, '改验证模式') > 0 THEN
  ...
  BEGIN
    execute immediate '
    MERGE INTO ix_poi p1
    USING (select au.* from au_ix_poi au ,temp_his_ix_poi_ext ext where au.pid=ext.pid  and ('||merge_utils.get_oprstatus_clause(v_merge_type,'au')||')) v
    ON (p1.pid = v.pid)
    WHEN MATCHED THEN
  ...
  BEGIN
    ...
    USING (SELECT p2.pid, p2.log, p2.state
            ...
            WHERE p2.state = 2
              AND (' ||
                      merge_utils.get_oprstatus_clause(v_merge_type, 'p2') || ')
              AND NOT EXISTS (SELECT 1
                     FROM temp_au_ix_poi_mul_task mul
  ...
  */
  ...
  PROCEDURE att_add_poi_kind_code_ext IS
    v_oprstatus_clause VARCHAR2(100);
  BEGIN
    v_oprstatus_clause := merge_utils.get_oprstatus_clause(merge_utils.merge_type_att,
                                                           'au');
     -- 通过POI_PID，删除IX_POI_FLAG表中FLAG_CODE 为 110000100000,110000120000,110000140000,110000150000,110000170000,110000220000 的记录，
    execute immediate '
    DELETE FROM ix_poi_flag t
     ...
     FROM au_ix_poi_name au, TEMP_HIS_IX_POI_EXT ext,au_ix_poi aui
      ...
      INSERT INTO temp_ix_poi_name_mg
        ...
        SELECT  pid_man.pid_nextval('IX_POI_NAME') AS name_id,
         ...
         WHERE au.poi_pid = rs.poi_pid(+) and aui.audata_id = au.audata_id 
  ...
  */
  ...
  BEGIN
    ...
    :4	修改整条中文地址记录的数据；
    :5	删除对应英文和葡文地址记录；
    :6	同时通过POI_PID删除IX_POI_FLAG表中FLAG_CODE 为以下值的记录，如果没有，则不处理；
    ...
    DELETE FROM ix_poi_address c1
     ...
     (SELECT 1 FROM temp_his_ix_poi_ext l WHERE c1.poi_pid = l.pid)      
      AND EXISTS (SELECT 1
                  FROM au_ix_poi au
                 WHERE au.pid = c1.poi_pid
                   AND au.att_oprstatus = 0 and exists(select 1 from au_ix_poi_address t where t.audata_id=au.audata_id))
       AND c1.lang_code IN ('CHI', 'CHT');
      --改名称 POI名称、POI拼音、OLD名称 (POINAME、POIPY)
      INSERT INTO ix_poi_address
        SELECT name_id,
       ...
       (SELECT 1 FROM temp_his_ix_poi_ext l WHERE c1.poi_pid = l.pid)
         AND NOT EXISTS (SELECT 1
               ...
               WHERE ipc.poi_pid = c1.poi_pid
                 AND ipc.contact = c1.contact
                 AND ipc.contact_type in (1,2))
      and c1.poi_pid = aui.pid and aui.att_oprstatus = 0 and c1.audata_id = aui.audata_id AND c1.contact_type in (1,2);
        /* AND EXISTS (SELECT 1
                FROM au_ix_poi au
               WHERE au.pid = c1.poi_pid
                 AND au.att_oprstatus = 0) and c1.audata_id = aui.audata_id;*/
  ...
  BEGIN
     ...
     INSERT INTO  TEMP_IX_POI_RESTAURANT_MG   
     SELECT * FROM IX_POI_RESTAURANT A WHERE EXISTS (
           SELECT 1 FROM  temp_his_ix_poi_ext l WHERE
           L.PID = A.POI_PID 
     );
  ...
  BEGIN
    ...
    :7  修改整条中文地址记录的数据；
    :8  删除对应英文和葡文地址记录；
    :9  同时通过POI_PID删除IX_POI_FLAG表中FLAG_CODE 为以下值的记录，如果没有，则不处理；
  ...
  BEGIN
       ...
       execute immediate ' INSERT INTO  TEMP_IX_POI_RESTAURANT_MG   
        SELECT * FROM IX_POI_RESTAURANT A WHERE EXISTS (
               SELECT 1 FROM  au_ix_poi l WHERE
               l.pid = a.poi_pid and l.audata_id = :v_audata_id
         )' USING v_audata_id;
 ...
 --通过POI_PID判断IX_POI_NAME表中Lang_Code为”CHI”、NAME_CLASS为官方名称（1），名称类型（NAME_TYPE）为原始（2）的记录是否存在，如果不存在，则增加一条记录；否则修改该记录的NAME和NAME_PHONETIC的值。同时更新IX_POI的old_name字段
  ...
  BEGIN   
    ...
    :10  修改整条中文地址记录的数据；
    :11  删除对应英文和葡文地址记录；
    :12  同时通过POI_PID删除IX_POI_FLAG表中FLAG_CODE 为以下值的记录，如果没有，则不处理；
    */
    DELETE FROM ix_poi_address c1
     WHERE EXISTS (SELECT 1
  ...
  BEGIN
    ...
    DELETE FROM ix_poi_children ipc
     WHERE EXISTS (SELECT 1
      ...
      --1.修改的是子，将父和子都copy到内业中
      merge into temp_ix_poi_parent_mg ipp
      ...
      SELECT group_id, child_poi_pid, relation_type, u_record, u_fields
        FROM (SELECT ipp.group_id,
               ...
               WHERE c.audata_id = v_audata_id
                 and (exists
                      (select 1
                         from au_ix_poi_parent auipp
                        where auipp.field_task_id = c.field_task_id
                          and auipp.group_id = c.group_id
                          and auipp.parent_poi_pid = ipp.parent_poi_pid) or
                      (c.group_id = ipp.group_id and not exists
                       (select 1
                          from au_ix_poi_parent t
                         where t.group_id = c.group_id
                           and t.field_task_id = c.field_task_id)))) rs
       WHERE rs.rn = 1;
  END;
  ...
  BEGIN
    DELETE FROM temp_ix_poi_restaurant_mg;
      INSERT INTO temp_ix_poi_restaurant_mg(RESTAURANT_ID,POI_PID,FOOD_TYPE,CREDIT_CARD,AVG_COST,PARKING,TRAVELGUIDE_FLAG,U_RECORD,U_FIELDS)
        SELECT restaurant_id,
               poi_pid,
               food_type,
               credit_card,
 ...
 */
   ...
   -- 融合时，如果AU_IX_POI表中，log字段包含“改内部POI”，那么融合后在IX_POI_FLAG中插入一条记录110000030000（如果存在，则不插入）
/*  EXECUTE IMMEDIATE 'INSERT INTO ix_poi_flag ipf
  SELECT au.poi_pid, ''110000030000'', 0 AS u_record, NULL AS u_fields
    FROM au_ix_poi_flag au
   WHERE au.audata_id=:v_audata_id 
     and au.flag_code=''110000030000''   
     AND NOT EXISTS (SELECT 1
            FROM ix_poi_flag ipf
           WHERE ipf.poi_pid = au.poi_pid
  ...
  procedure mul_reset_poi_parent(v_audata_id number)is
   ...
   procedure process_mod_poi_name_delclass5 is
        ...
        begin
          --如果母库IX_POI_FLAG中存在FLAG_CODE为“110000240000”的记录，则删除IX_POI_FLAG中该记录，如果没有，则不处理，不报错。
           ...
           WHERE t.flag_code = '110000240000'
             AND EXISTS (SELECT 1
                    FROM au_ix_poi tmp
                   WHERE tmp.pid = t.poi_pid and tmp.att_oprstatus=0);
          end;
    ...
    END;
     ...
     END;
     /*
     通过POI_PID关联，使用AU_IX_POI表VERIFIED_FLAG修改IX_POI表的VERIFIED_FLAG字段
     */
    ...
    BEGIN
     ...
     PROCEDURE process_att_geoatt_samepoi(v_merge_type VARCHAR2) IS
     	v_oprstatus_clause VARCHAR2(100);
     BEGIN
	    v_oprstatus_clause := merge_utils.get_oprstatus_clause(v_merge_type,'a');
