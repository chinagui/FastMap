...
public class Process extends AbstractProcess<Command> {
    ...
    public boolean prepareData() throws Exception {
        ...
        try {
            ...
            // 获取该link关联的限高限重
            RdHgwgLimitSelector rdHgwgLimitSelector = new RdHgwgLimitSelector(getConn());
            List<RdHgwgLimit> hgwgLimits = rdHgwgLimitSelector.loadByLinkPid(this.getCommand().getLinkPid(), true);
            getCommand().setRdHgwgLimits(hgwgLimits);
            return true;
    ...
    private void updataRelationObj() throws Exception {
        ...
        // 维护限高限重
        OpRefRdHgwgLimit opRefRdHgwgLimit = new OpRefRdHgwgLimit(getConn());
        opRefRdHgwgLimit.run(getResult(), oldLink, getCommand().getNewLinks());
        long refRdHgwgLimitTime = System.currentTimeMillis();
        log.info("限高限重USE TIME  " + String.valueOf(refRdHgwgLimitTime - refSameTime));
    ...
    private Map<String, List<Integer>> confirmRelationObj() throws Exception {
        ...
        // 限高限重
        infectList = new ArrayList<Integer>();
        for (RdHgwgLimit rdHgwgLimit : getCommand().getRdHgwgLimits()) {
            infectList.add(rdHgwgLimit.pid());
        }
