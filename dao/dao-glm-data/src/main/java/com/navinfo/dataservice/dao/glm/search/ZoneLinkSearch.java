...
public class ZoneLinkSearch implements ISearch {
	...
	public List<SearchSnapshot> searchDataByLinkPids(List<Integer> pids)
		...
		String sql = "WITH TMP1 AS (SELECT LINK_PID, GEOMETRY, S_NODE_PID, E_NODE_PID FROM ZONE_LINK WHERE U_RECORD != 2 AND LINK_PID IN ("
				+ ids
				+ ")) SELECT A.*, (SELECT COUNT(1) FROM ZONE_LINK_KIND Z WHERE Z.LINK_PID = A.LINK_PID AND Z.U_RECORD != 2 AND Z.KIND = 1) AOI, (SELECT COUNT(1) FROM ZONE_LINK_KIND Z WHERE Z.LINK_PID = A.LINK_PID AND Z.U_RECORD != 2 AND Z.KIND = 2) KD FROM TMP1 A";
