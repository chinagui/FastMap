 ...
 * @author songdongyan
 * @date 2016年12月29日
 * @Description: 检查原则：警示信息不能够制作在9级辅路上。否则报LOG 道路属性编辑后检查 RdLinkForm(修改或者新增)
 *               link种别编辑后检查 RdLink修改 新增警示信息后检查 RdWarningInfo 道路属性编辑前检查
 *               RdLinkForm(修改或者新增) link种别编辑前检查 RdLink修改
 */
public class GLM09019 extends baseRule {
	...
	private void checkRdLink(RdLink rdLink, CheckCommand checkCommand) throws Exception {
		...
		if (rdLink.changedFields().containsKey("kind")) {
			...
			if (kind == 9) {
				...
				sb.append("SELECT 1 FROM RD_LINK_FORM F,RD_LINK_WARNING RW ");
				...
				String sql = sb.toString();
				log.info("RdLink后检查GLM09019:" + sql);
				...
				if (resultList.size() > 0) {
					...
					this.setCheckResult(rdLink.getGeometry(), target, 0);
				}
			}
		}
	}
	...
	private void checkRdRdLinkForm(RdLinkForm rdLinkForm) throws Exception {
		...
		if (formOfWay == 34) {
			...
			sb.append("SELECT 1 FROM RD_LINK R,RD_LINK_WARNING RW ");
			...
			String sql = sb.toString();
			log.info("RdLinkForm后检查GLM09019:" + sql);
