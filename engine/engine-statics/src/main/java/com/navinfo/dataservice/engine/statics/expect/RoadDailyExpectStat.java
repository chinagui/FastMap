...
public class RoadDailyExpectStat implements Runnable {
	private Logger log = LogManager.getLogger(RoadDailyExpectStat.class);
	...
	public RoadDailyExpectStat(CountDownLatch cdl, List<BlockMan> blocks,
	...
	private Map<String, Double> getBlockStat(String blockId) {
		...
		MongoCursor<Document> iter = md.find(RoadDailyMain.col_name_block,
	...
	private List<Document> doStat() {
		...
		Map<String, Integer> mapSeason = StatInit.getTipsFinishOfSeason(
				db_name, RoadDailyMain.col_name_seasion_block, "block_id");

		for (BlockMan block : blocks) {
			if (block.getDayEditPlanStartDate() == null
					|| block.getDayEditPlanEndDate() == null) {
				continue;
			}

			String blockId = block.getBlockId().toString();

			Timestamp startDate = (Timestamp) block.getDayEditPlanStartDate();

			Timestamp endDate = (Timestamp) block.getDayEditPlanEndDate();
