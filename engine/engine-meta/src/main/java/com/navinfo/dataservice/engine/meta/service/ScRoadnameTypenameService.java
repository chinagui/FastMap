...
/** 
* @ClassName:  ScRoadnameTypenameService 
* @author code generator
* @date 2017-03-23 07:06:35 
* @Description: TODO
*/
@Service
public class ScRoadnameTypenameService {
	private Logger log = LoggerRepos.getLogger(this.getClass());

	
	public void create(ScRoadnameTypename  bean)throws ServiceException{
		...
		try{
			...
			String createSql = "insert into SC_ROADNAME_TYPENAME (ID, NAME, PY, ENGLISHNAME, REGION_FLAG, LANG_CODE) values(?,?,?,?,?,?)";			
			run.update(conn, 
					   createSql, 
	...
	public void update(ScRoadnameTypename bean)throws ServiceException{
		Connection conn = null;
		try{
			//持久化
			QueryRunner run = new QueryRunner();
			...
			String updateSql = "update SC_ROADNAME_TYPENAME set ID=?, NAME=?, PY=?, ENGLISHNAME=?, REGION_FLAG=?, LANG_CODE=? where 1=1 ";
	...
	public void delete(ScRoadnameTypename bean)throws ServiceException{
		Connection conn = null;
		try{
			//持久化
			QueryRunner run = new QueryRunner();
			...
			String deleteSql = "delete from  SC_ROADNAME_TYPENAME where 1=1 ";
			List<Object> values=new ArrayList<Object>();
			if (bean!=null&&bean.getId()!=null && StringUtils.isNotEmpty(bean.getId().toString())){
				deleteSql+=" and ID=? ";
				values.add(bean.getId());
	...
	public Page list(ScRoadnameTypename bean ,final int currentPageNum)throws ServiceException{
		Connection conn = null;
		try{
			QueryRunner run = new QueryRunner();
			conn = DBConnector.getInstance().getMetaConnection();
			
			String selectSql = "select * from SC_ROADNAME_TYPENAME where 1=1 ";
			List<Object> values=new ArrayList<Object>();
			if (bean!=null&&bean.getId()!=null && StringUtils.isNotEmpty(bean.getId().toString())){
				selectSql+=" and ID=? ";
				values.add(bean.getId());
			...
			ResultSetHandler<Page> rsHandler = new ResultSetHandler<Page>(){
				public Page handle(ResultSet rs) throws SQLException {
					List<ScRoadnameTypename> list = new ArrayList<ScRoadnameTypename>();
		            Page page = new Page(currentPageNum);
					while(rs.next()){
						ScRoadnameTypename model = new ScRoadnameTypename();
	...
	public Page list(JSONObject dataJson,final int curPageNum, final int pageSize, String sortby) throws ServiceException{
		...
		try{
			QueryRunner run = new QueryRunner();
			conn = DBConnector.getInstance().getMetaConnection();
			ScRoadnameTypename bean =(ScRoadnameTypename) JSONObject.toBean(dataJson, ScRoadnameTypename.class);
	...
	public List<ScRoadnameTypename> list(ScRoadnameTypename bean)throws ServiceException{
		Connection conn = null;
		try{
			QueryRunner run = new QueryRunner();
			conn = DBConnector.getInstance().getMetaConnection();	
			...
			ResultSetHandler<List<ScRoadnameTypename>> rsHandler = new ResultSetHandler<List<ScRoadnameTypename>>(){
				public List<ScRoadnameTypename> handle(ResultSet rs) throws SQLException {
					List<ScRoadnameTypename> list = new ArrayList<ScRoadnameTypename>();
					while(rs.next()){
						ScRoadnameTypename model = new ScRoadnameTypename();
						model.setId(rs.getInt("ID"));
	...
	public ScRoadnameTypename query(ScRoadnameTypename bean)throws ServiceException{
		Connection conn = null;
		try{
			//持久化
			QueryRunner run = new QueryRunner();
			...
			ResultSetHandler<ScRoadnameTypename> rsHandler = new ResultSetHandler<ScRoadnameTypename>(){
				public ScRoadnameTypename handle(ResultSet rs) throws SQLException {
					while(rs.next()){
						ScRoadnameTypename model = new ScRoadnameTypename();
						model.setId(rs.getInt("ID"));
						model.setName(rs.getString("NAME"));
	...
	public void saveUpdate(JSONObject dataJson) throws ServiceException {
		...
		if(bean.getName() != null && StringUtils.isNotEmpty(bean.getName()) 
			...
			ScRoadnameTypename newBean = new ScRoadnameTypename();
			newBean.setName( bean.getName());
			newBean.setLangCode(bean.getLangCode());
			ScRoadnameTypename oldBean = query(newBean);
