 ...
 * 若查找不到或找到的结果与行政等级对照表不一致的POI，全部报出。 
 * 见《备注》中行政等级对照表 提示：地址行政区域字段检查： 
 * ① 若省名、市名、区县名在“地址拆分用行政区划对照表”中找不到，则报：省名、市名、区县名在 “地址拆分用行政区划对照表”中找不到；
 * ② 若省名、市名、区县名在“地址拆分用行政区划对照表”中能找到，但与行政区划等级不一致，
 * 则将配置表中的行政区划号（ADMIN_ID）和行政区划名称名称（ADMIN_NAME）报出来；
 *
 */
	...
	public void runCheck(BasicObj obj) throws Exception {
		...
		if (StringUtils.isNotEmpty(province)) {
			if (addrAdminMap.containsKey(province)) {
				...
				errList.add("省名在 “地址拆分用行政区划对照表”中找不到；");
			}
		}
		...
		if (StringUtils.isNotEmpty(city)) {
			if (addrAdminMap.containsKey(city)) {
				...
				errList.add("市名在 “地址拆分用行政区划对照表”中找不到；");
			}
		}
		...
		if (StringUtils.isNotEmpty(county)) {
			if (addrAdminMap.containsKey(county)) {
				...
				errList.add("区县名在 “地址拆分用行政区划对照表”中找不到；");
			}
		}
