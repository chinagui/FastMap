...
*	1.网址信息以http://开头                                  log:网址信息格式错误，网址不是以”http://”开头
*	2.网址信息不能存在空格，tab符，回车符          log:网址信息格式错误，网址中存在Tab符、回车符或者空格
*	3.网址信息不能为http://* /*格式                log:网址信息格式错误，网址中存在多余的“/”
*	4.网址信息不以“\”结尾                 			 log:网址信息格式错误，网址以“\”结尾
*/
				...
				if (poiDetails.size() > 0){
					for (IRow detail: poiDetails){
						...
						if (!webSite.isEmpty()){
							...
							String webSiteEnd = webSite.substring(7);
							...
							if (log.length() > 0){
								...
								this.setCheckResult(poi.getGeometry(), "[IX_POI,"+poi.getPid()+"]", poi.getMeshId(), checkLog);
							}
						}
					}
				}
