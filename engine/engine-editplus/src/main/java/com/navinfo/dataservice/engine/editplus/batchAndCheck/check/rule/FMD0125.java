...
/**
 * FM-D01-25 别名中文名称繁体字检查  D
 * 检查条件：
 * 非删除POI对象
 * 检查原则：
 * 别名中文名称不能有繁体字。查找的繁体字在TY_CHARACTER_FJT_HZ中所在行的CONVERT字段的值：
 * 1、如果是1表示不转化，不用报log；
 * 2、如果是0表示需要确认后转化，报log：**是繁体字，对应的简体字是**，需确认是否转化；
 * 检查名称：标准化中文名称（name_type=1，name_class={3}，lang_code=CHI)
 * @author gaopengrong
 */
public class FMD0125 extends BasicCheckRule {
	...
	public void runCheck(BasicObj obj) throws Exception {
		if(obj.objName().equals(ObjectName.IX_POI)){
			...
			for(IxPoiName nameTmp:names){
				if(nameTmp.getLangCode().equals("CHI")&&nameTmp.getNameType()==1&&nameTmp.getNameClass()==3){
