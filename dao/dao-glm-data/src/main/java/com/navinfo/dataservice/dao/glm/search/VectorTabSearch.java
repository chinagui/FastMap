...
public class VectorTabSearch  implements ISearch{
    ...
    public VectorTabSearch(Connection conn) {
        this.conn = conn;
    }
	...
	List<SearchSnapshot> list = new ArrayList<>();
	    	...
	    	String sql = "SELECT A.PID,A.NAME, A.GEOMETRY,A.KIND,A.WIDTH,A.MULTI,A.DERECT,A.FORM  " 
	    					+" FROM vector_tab A "
	    					+"    WHERE  "
	        ...
	        try {
				...
				while (resultSet.next()) {
					...
					m.put("b", resultSet.getString("KIND"));
	
					m.put("c", resultSet.getString("WIDTH"));
	
					m.put("d", resultSet.getString("MULTI"));
	
					m.put("e", resultSet.getString("DERECT"));
	
					m.put("f", resultSet.getString("FORM"));
