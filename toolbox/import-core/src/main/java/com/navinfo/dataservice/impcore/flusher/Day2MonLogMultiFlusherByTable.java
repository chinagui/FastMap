 ...
 */
public class Day2MonLogMultiFlusherByTable extends FlushLogBySQL {
	private Logger log = LoggerRepos.getLogger(this.getClass());
	private OracleSchema dailyDbSchema;

	public Day2MonLogMultiFlusherByTable(OracleSchema dailyDbSchema,
	...
	protected void calcConcurrentSize() throws SQLException {
		int tabSize = this.getLogCount(this.tempTable);
		log.debug("tabSize ==== " + tabSize);
		if (tabSize == 0) {
			return;
		}
		if (tabSize <= 20) {
			concurrentSize = dataCount;
		} else {
			concurrentSize = 20;
		}
	}
	...
	public String getLogQuerySql(int innerCount, String tempTable) {
		...
		String queryLogSqlInner = "select L.* from LOG_DETAIL L, " + tempTable + " T       \n"
	...
	public int getLogCount(String tempTable) throws SQLException {
		...
		int count = 0;
		String queryLogCountSql = "select count(distinct tb_nm) count \n"
