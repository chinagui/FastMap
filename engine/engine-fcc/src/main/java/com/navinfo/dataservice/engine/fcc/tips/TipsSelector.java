...
public class TipsSelector {
	...
	public JSONArray searchPoiRelateTips(String id, int subTaskId, int buffer, int dbId) throws Exception {
		...
		String unhandleGps = String.format(
				"SELETE * FROM TIPS_INDEX WHERE T_TIPSTATUS = 2 AND S_QSUBTASKID = %d AND S_SOURCETYPE = 2001 AND STAGE IN (1,2,5,6) AND ID = %s",
				subTaskId, id);

		// B、库中状态为已处理且没有形状删除的测线tips
		String handleGps = String.format(
				"SELETE * FROM TIPS_INDEX WHERE T_DEDITSTATUS = 2 AND S_QSUBTASKID = %d AND S_SOURCETYPE = 2001 AND STAGE = 2 AND ID = %s",
				subTaskId, id);

		// C、形状删除的Tips关联的link且该link存在或逻辑删除，且该POI的引导link是该link的pid
		String deleteLinks = String.format(
				"SELETE * FROM TIPS_INDEX WHERE S_QSUBTASKID = %d AND S_SOURCETYPE = 2101 AND ID = %s", subTaskId, id);
