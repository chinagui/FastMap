 ...
 * (2)别名原始英文(name_class=3,name_type=2,lang_code='ENG')大于35且为重要分类(参考SC_POINT_SPEC_KINDCODE_NEW中TYPE=8)
 * 批处理：
 * 满足条件(1)时，删除别名标准英文记录；
 * 满足条件(2)时，如果该组存在别名标准化英文，则更新别名标准英文name值；如果不存在别名标准英文记录，则需要新增一条记录(name_id申请赋值，name_groupid与同组中文别名一致，lang_code与同组中文别名一致，name_class=3，name_type=1)
 * @author jch
 */
