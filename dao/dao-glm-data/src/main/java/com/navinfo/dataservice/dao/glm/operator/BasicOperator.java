...
public class BasicOperator extends AbstractOperator {
		...
		for (Field f : fields) {
			if (isBasicType(f)) {
				...
				if (StringUtils.toColumnName(name).equals(M_U_DATE)) {
					...
					/*
					 * String tableName = SelectorUtils.getObjTableName(row); if
					 * (StringUtils.isNotEmpty(row.rowId())) { if
					 * (!tableName.equals("IX_POI")) {
					 * row.setRowId(UuidUtils.genUuid()); } } else {
					 * row.setRowId(UuidUtils.genUuid()); } logger.info("rowid:"
					 * + row.rowId()); System.out.println("poi rowid:" +
					 * row.rowId());
					 */
			...
			if (value instanceof String || value == null) {
				...
				if (!StringUtils.isStringSame(oldWkt, newWkt)) {
					if (newWkt.length() > 4000) {
						sb.insert(0,
								"DECLARE v_geo SDO_GEOMETRY := sdo_geometry('"
										+ newWkt.trim() + "',8307); BEGIN ");
						sb.append("geometry=v_geo,");
						complexGeo = true;
					} else {
	...
	private void addConditionForPoi(StringBuilder sb) throws SQLException {
		...
		if (tableName.equals("IX_POI") || row instanceof IxPoi
				|| row instanceof IxPoiChildren || row instanceof IxPoiParent) {
