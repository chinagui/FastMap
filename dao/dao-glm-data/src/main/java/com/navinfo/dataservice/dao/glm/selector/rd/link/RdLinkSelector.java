...
public class RdLinkSelector extends AbstractSelector {
	 ...
	 * 仅加载主表RDLINK，其他子表若有需要，请单独加载
	 */
	public List<RdLink> loadByNodePids(List<Integer> nodePids, boolean isLock)
			throws Exception {
		List<RdLink> links = new ArrayList<RdLink>();
	 ...
	 * 仅加载主表RDLINK，其他子表若有需要，请单独加载
	 */
	private List<RdLink> loadByNodePid(List<Integer> nodePids, boolean isLock)
			throws Exception {
	...
	public List<RdLink> loadTrackLinkNoDirect(int linkPid, int nodePidDir,
			boolean isLock) throws Exception {
		List<RdLink> list = new ArrayList<RdLink>();
		StringBuilder sb = new StringBuilder();
	...
	public List<RdLink> loadLinkPidByNodePids(String nodePidsStr, boolean isLock)
		...
		try {
			StringBuilder sb = new StringBuilder(
					"select link_pid,SPECIAL_TRAFFIC,IMI_CODE from rd_link where S_NODE_PID in("
			...
			while (resultSet.next()) {
				...
				List<IRow> forms = new AbstractSelector(RdLinkForm.class, conn)
						.loadRowsByParentId(link.getPid(), isLock);
	...
	private void setChildData(RdLink rdLink, boolean isLock) throws Exception {
		...
		// rd_tmclocation_link
		List<IRow> tmcLocationLinks = new AbstractSelector(
				RdTmclocationLink.class, conn).loadRowsByParentId(
				rdLink.getPid(), isLock);
	...
	public List<RdLink> loadLinkByFaceGeo(int facePid, String tableName,
		...
		try {
			...
			while (resultSet.next()) {
				...
				List<IRow> zones = this.loadRowsByClassParentId(
						RdLinkZone.class, link.getPid(), true, null);
	 ...
	 * @throws Exception
	 */
	public List<RdLink> loadLinkByFaceGeo(Geometry geometry, boolean isLock)
			throws Exception {
		...
		try {
			...
			while (resultSet.next()) {
				...
				ReflectionAttrUtils.executeResultSet(link, resultSet);
				List<IRow> zones = new AbstractSelector(RdLinkZone.class, conn)
						.loadRowsByParentId(link.pid(), true);
				link.setZones(zones);
				rdLinks.add(link);
			}
		} catch (Exception e) {
	 ...
	 * @throws Exception
	 */
	public List<RdLink> loadLinkByDiffGeo(Geometry sourceGeo, Geometry geo,
			boolean isLock) throws Exception {
		List<RdLink> rdLinks = new ArrayList<>();
	 ...
	 * @throws Exception
	 */
	public List<IRow> loadRdTmcByLinkPid(int linkPid, boolean isLock)
			throws Exception {
		...
		try {
			...
			while (resultSet.next()) {
				...
				List<IRow> tmcLinks = new AbstractSelector(conn)
						.loadRowsByClassParentId(RdTmclocationLink.class,
								tmclocation.getPid(), isLock, null, null);

				tmclocation.setLinks(tmcLinks);

				for (IRow row : tmcLinks) {
					tmclocation.linkMap.put(row.rowId(),
							(RdTmclocationLink) row);
				}
	...
	public List<Integer> loadRdLinkKindByIds(List<Integer> linkPidList,
			boolean isLock) throws Exception {
		List<Integer> kinds = new ArrayList<Integer>();

		StringBuilder sb = new StringBuilder(
				"select kind from rd_link where link_pid in("
						+ StringUtils.join(linkPidList, ",") + ")");
