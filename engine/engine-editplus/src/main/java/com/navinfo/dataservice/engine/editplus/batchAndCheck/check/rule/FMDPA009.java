 ...
 * @date: 2017年10月14日
 * @Desc: FMDPA009.java 检查条件： 非删除点门牌对象 检查原则：
 *        DP_NAME字段不能有繁体字。查找的繁体字在TY_CHARACTER_FJT_HZ中所在行的CONVERT字段的值是0，
 *        则DP_NAME不能是FT对应的值，否则报log：外业门牌号＊＊是繁体字，对应的简体字是＊＊，请确认是否正确！
 */
public class FMDPA009 extends BasicCheckRule {
	...
	public void run() throws Exception {
		...
		for (Long key : rows.keySet()) {
			...
			if (obj.objName().equals(ObjectName.IX_POINTADDRESS) && !obj.opType().equals(OperationType.PRE_DELETED)) {
				...
				String dpName = ixPonitaddress.getDpName();
				...
				if (!fts.isEmpty()) {
					...
					String log = "外业门牌号" + ftsString + "是繁体字，对应的简体字是" + jtsString + "，请确认是否正确！";
