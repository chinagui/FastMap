...
public abstract class DataBaseUtils {
	 ...
	 * @return
	 */
	public static List<ColumnMetaData> getTableMetaData(Connection conn,
			String tableName, boolean closeConn) throws Exception {
	...
	public static String clob2String(CLOB clob) throws SQLException {
		String reString = (clob != null ? clob.getSubString(1,
	...
	public static String generateInsertSql(String tableName,
			List<ColumnMetaData> tmdList, List<String> excludeColumnList,
			boolean noValue) {
	...
	public static StringBuilder generateInsertNoValuesSql(String tableName,
			List<ColumnMetaData> tmdList, List<String> excludeColumnList) {
		return new StringBuilder(generateInsertSql(tableName, tmdList,
				excludeColumnList, true));
	}

	public static List<ColumnMetaData> removeIgnoreColumn(
			List<ColumnMetaData> tmdList, List<String> ignoreColumns) {
	 ...
	 * @return
	 */
	public static String generateSimpleQueryString(String tableName,
			List<ColumnMetaData> tmdList) {
		long t1 = System.currentTimeMillis();
		...
		builder.append(" l ");
		/*
		 * if (hasMeshColumn) { builder.append(" where mesh_id =595673"); }
		 */
	 ...
	 * @throws Exception
	 */
	public static List<String> getTables(String tableNameLike, Connection conn,
	 ...
	 * @return
	 */
	public static DataSource getSpecifiedDataSource(JdbcTemplate jdbcTemplate,
			String key) {
		DataSource dataSource = jdbcTemplate.getDataSource();
		if (dataSource instanceof DynamicDataSource) {
			dataSource = ((DynamicDataSource) dataSource)
					.getSpecifiedDataSource(key);
		}
	 ...
	 * @return
	 */
	public static Connection getSpecifiedConnection(JdbcTemplate jdbcTemplate,
			String key) throws SQLException {
		Connection conn = getSpecifiedDataSource(jdbcTemplate, key)
				.getConnection();
	...
	public static List<String> getForeignKeys(Connection connection,
			String tableName) throws SQLException {
		List<String> keyList = new ArrayList<String>();
		DatabaseMetaData meta = connection.getMetaData();
	...
	public static void turnOnPkConstraint(Connection conn,
			Set<String> tableNames) throws SQLException {
		switchConstraint(conn, true, "P", tableNames);

	}

	public static void turnOffPkConstraint(Connection conn,
			Set<String> tableNames) throws SQLException {
		switchConstraint(conn, false, "P", tableNames);

	}

	public static void turnOnFkConstraint(Connection conn,
			Set<String> tableNames) throws SQLException {
		switchConstraint(conn, true, "F", tableNames);

	}

	public static void turnOffFkConstraint(Connection conn,
			Set<String> tableNames) throws SQLException {
		switchConstraint(conn, true, "F", tableNames);

	}

	private static void switchConstraint(Connection conn, final boolean onOff,
			String constraintType, Set<String> targetTables)
			throws SQLException {
		...
		String tableSql = "select table_name,constraint_name from user_constraints c where c.constraint_type='"
				+ constraintType + "' AND C.TABLE_NAME NOT LIKE 'BIN%' ";

		tableSql += " AND table_name in ('"
				+ StringUtils.join(targetTables, "','") + "')";

		QueryRunner runner = new QueryRunner();
		// KEY=CONSTRAINT_NAME,VALUE=TABLE_NAME
		Map<String, String> tables = runner.query(conn, tableSql,
				new ResultSetHandler<Map<String, String>>() {
					...
					public Map<String, String> handle(ResultSet rs)
						...
						while (rs.next()) {
							tables.put(
									new String(rs.getString("CONSTRAINT_NAME")),
									new String(rs.getString("TABLE_NAME")));
						}
						return tables;
					}
		...
		for (String constraintName : tables.keySet()) {
			String primaryKeySql = "ALTER TABLE " + tables.get(constraintName)
	...
	/**
	 * @param conn
	 *            传入的链接，本方法不关闭；调用放负责关闭conn
	 * @param seqName
	 *            ORACLE sequence 的名称
	 * @return sequnce.NEXTVAL
	 * @throws Exception
	 */
	public static long fetchSequence(Connection conn, String seqName)
			throws Exception {
		QueryRunner runner = new QueryRunner();
		...
		return runner.query(conn, "select " + seqName + ".NEXTVAL  from dual",
				rsh);
	}
	 ...
	 */
	public static boolean isTableExists(DataSource ds, String tableName)
			throws SQLException {
		int maxOperateLogId = 0;
		String sql = "SELECT count(1) FROM user_tables O WHERE O.table_name = ? ";
		QueryRunner run = new QueryRunner();
		maxOperateLogId = run.queryForInt(ds, sql, tableName.toUpperCase());
		return maxOperateLogId == 1;
	}

	public static void main(String args[]) {
		try {
			String name = DataBaseUtils
					.getReturnTable("SELECT P.* FROM POI_TELE P,TEMP_POI_INFO T WHERE P.POI_PID=T.PID");
