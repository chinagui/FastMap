...
public class Operation implements IOperation {
	 ...
	 * @param limitContent
	 * @param direct
	 * @return limitInfo[4] 0：限速值；1：限速来源；2：限速等级3：限速等级标记
	 */
	private int[] getLimitInfo(JSONObject limitContent, int direct) {

		int[] limitInfo = new int[4];
		...
		limitInfo[2] = getspeedClass(limitInfo[1]);
	...
	private String batch(Result result, List<Integer> linkPids,
		...
		// limitInfo[4] 0：限速值；1：限速来源；2：限速等级3：限速等级标记
	...
	private void SetSpeedLimit(Result result, int linkPid,
		...
		if (isCreate) {
			...
			limit.setSpeedClass(info[2]);
			limit.setSpeedClassWork(info[3]);
			...
			if (limit.getSpeedClass() != info[2]) {
				limit.changedFields().put("speedClass", info[2]);
			}
			if (limit.getSpeedClassWork() != info[3]) {
				limit.changedFields().put("speedClassWork", info[3]);
			}
