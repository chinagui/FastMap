...
public class EdgeMatchTipsOperator extends BaseTipsOperate {
	 ...
	 */
	public String create(JSONObject g_location, String content, int user,
			String memo, int qSubTaskId) throws Exception {
		...
		try {
			...
			// solr index json
			TipsDao tipsIndexModel = TipsUtils.generateSolrIndex(rowkey, stage,
					currentDate, user, trackJson, sourceJson, jsonGeom,
					new JSONObject(), feedbackObj);
			conn = DBConnector.getInstance().getTipsIdxConnection();
			TipsIndexOracleOperator indexOracleOperator = new TipsIndexOracleOperator(
					conn);
			indexOracleOperator.save(tipsIndexModel);
