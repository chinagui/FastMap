...
public class RowChargeController extends BaseController{
	...
	public ModelAndView getChargePoi(HttpServletRequest request) throws Exception {
		try {
...
//			Long userId = 0L;
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/photo/charge/download")
	public ModelAndView getChargePhoto(HttpServletRequest request) throws Exception {
		try {
			String parameter = request.getParameter("parameter");
			if (StringUtils.isEmpty(parameter)) {
				throw new IllegalArgumentException("parameter参数不能为空。");
			...
			log.info("editRow/photo/charge/download接口参数:"+paraJson.toString());
			if(!paraJson.containsKey("type")){
				throw new IllegalArgumentException("parameter参数中type不能为空。");
			}
			int type = paraJson.getInt("type");
			...
			RowChargeService service = new RowChargeService();
			String data = service.chargePhotoConvertor(type,lastSyncTime,syncTime,dbIdList);
			return new ModelAndView("jsonView", success(data));
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			return new ModelAndView("jsonView", fail(e.getMessage()));
