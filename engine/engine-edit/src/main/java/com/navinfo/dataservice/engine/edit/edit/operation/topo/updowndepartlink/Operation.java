...
public class Operation implements IOperation {
	...
	private void createDepartLinks(Result result) throws Exception {
		...
		for (int i = 0; i < links.size(); i++) {
			lineStrings[i] = (new GeometryFactory().createLineString(links
					.get(i).getGeometry().getCoordinates()));
		}
		// 调用分离后生成的上下线
	...
	private List<RdLink> createUpRdLink(RdLink departLink, Result result,
		...
		//对于上(左)线需要生成新的node
		if (slinks.size() <= 0) {
			this.getNodeByDepartGeo(departLink, 1,map ,result);
		}
		//如果对应起点有挂接的link
		...
		//对于上(左)线需要生成新的node
		if (elinks.size() <= 0) {
			this.getNodeByDepartGeo(departLink, 0, map, result);
		} 
		//如果对应终点有挂接的link
		//且没有下线挂接的link 需要修改原有node的属性
		//如果至少有一条下挂的link对于上(左)线需要生成新的node
	...
	private List<RdLink> createDownRdLink(RdLink departLink, Result result,
		...
		if (slinks.size() <= 0) {
			...
			this.getDepartRdlinkNode(slinks, departLink, sourceLink, sourceNextLink, 1, 0, map, result);
		}
		if (elinks.size() <= 0) {
			eNode = this.updateAdNodeForTrack(departLink,
					sourceLink.geteNodePid(), conn, result, 0);
		} else {
			this.getDepartRdlinkNode(elinks, departLink, sourceLink, sourceNextLink, 0, 0, map, result);
		}
		return RdLinkOperateUtils.addRdLink(sNode, eNode, departLink,
				sourceLink, result);
	}
