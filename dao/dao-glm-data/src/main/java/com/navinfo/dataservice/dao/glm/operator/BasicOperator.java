...
import static com.navinfo.dataservice.commons.geom.GeoTranslator.jts2Wkt;
        ...
        for (Field f : fields) {
            if (isBasicType(f)) {
                ...
                if (oj instanceof Geometry) {
                    String wkt = jts2Wkt((Geometry) oj, 0.00001, 5);
    ...
    public void updateRow2Sql(Statement stmt) throws Exception {
        ...
        while (it.hasNext()) {
            ...
            if (value instanceof String || value == null) {
                ...
                String oldWkt = jts2Wkt((Geometry) value, 0.00001, 5);

                String newWkt = GeoTranslator.jts2Wkt(GeoTranslator.geojson2Jts(JSONObject.fromObject(columnValue), 1, 5));
    ...
    public String getWhereForTable(IRow row) {
        try {
            if (row.getClass().getMethod(M_PRIMARYKEY).invoke(row) != null) {
                return "" + row.getClass().getMethod("M_PRIMARYKEY").invoke(row) + " = " + row.getClass().getMethod(M_PID).invoke(row) + "";
            } else {
