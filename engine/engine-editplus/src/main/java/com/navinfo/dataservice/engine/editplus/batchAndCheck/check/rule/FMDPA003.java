 ...
 * @Description:检查条件：
					非删除点门牌对象：
				检查原则：
					DP_NAME字段中允许存在“TY_CHARACTER_EGALCHAR_EXT”表，“EXTENTION_TYPE”字段中“GBK”、“ENG_F_U”、“ENG_F_L”、“DIGIT_F”、“SYMBOL_F”类型对应的“CHARACTER”字段的内容，如果存在其他字符，将此条点门牌报log：外业门牌号中含有非法字符“xx”！
				备注：检查时，先把name转成全角，再查
 * @Author: LittleDog
 * @Date: 2017年10月14日
 * @Version: V1.0
 */
public class FMDPA003 extends BasicCheckRule {
		...
		for (Map.Entry<Long, BasicObj> entryRow : getRowList().entrySet()) {
			...
			if (basicObj.objName().equals(ObjectName.IX_POINTADDRESS)) {
				...
				if (!StringUtils.isEmpty(ixPointaddress.getDpName())) {
					...
					dpName = CheckUtil.strB2Q(dpName);

					List<String> errorList = new ArrayList<String>();
					for (char nameSub : dpName.toCharArray()) {
					...
					if(errorList != null && errorList.size() != 0){
						setCheckResult(ixPointaddress.getGeometry(),
								...
								"外业门牌号中含有非法字符“" + errorList.toString().replace("[", "").replace("]", "") + "”!");
					}
				}
			}
		}
