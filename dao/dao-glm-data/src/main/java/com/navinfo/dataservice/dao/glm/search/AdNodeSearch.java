...
public class AdNodeSearch implements ISearch {
	...
	public List<SearchSnapshot> searchDataBySpatial(String wkt) throws Exception {
		...
		String sql = "WITH TMP1 AS (SELECT NODE_PID, FORM, GEOMETRY FROM AD_NODE WHERE SDO_WITHIN_DISTANCE(GEOMETRY, SDO_GEOMETRY(:1, 8307), 'mask=anyinteract') = 'TRUE' AND U_RECORD != 2), TMP2 AS (SELECT /*+ index(a) */ B.NODE_PID, LISTAGG(A.LINK_PID, ',') WITHIN GROUP(ORDER BY B.NODE_PID) LINKPIDS, LISTAGG(A.KIND, ',') WITHIN GROUP(ORDER BY B.NODE_PID) IMICODES, LISTAGG(A.FORM, ',') WITHIN GROUP(ORDER BY B.NODE_PID) LINK_FORMS FROM AD_LINK A, TMP1 B WHERE A.U_RECORD != 2 AND (A.S_NODE_PID = B.NODE_PID OR A.E_NODE_PID = B.NODE_PID) GROUP BY B.NODE_PID), TMP3 AS (SELECT /*+ index(a) */ B.NODE_PID, LISTAGG(A.GROUP_ID, ',') WITHIN GROUP(ORDER BY B.NODE_PID) SAMNODEPART FROM TMP1 B LEFT JOIN RD_SAMENODE_PART A ON B.NODE_PID = A.NODE_PID AND A.TABLE_NAME = 'AD_NODE' AND A.U_RECORD != 2 GROUP BY B.NODE_PID, A.GROUP_ID), TMP4 AS (SELECT /*+ index(a) */ B.NODE_PID, LISTAGG(A.PID, ',') WITHIN GROUP(ORDER BY B.NODE_PID) AS INTERNODE FROM TMP1 B LEFT JOIN RD_INTER_NODE A ON B.NODE_PID = A.NODE_PID AND A.U_RECORD != 2 GROUP BY B.NODE_PID) SELECT A.NODE_PID, A.GEOMETRY, A.FORM, B.LINKPIDS, B.LINK_FORMS, B.IMICODES, C.SAMNODEPART, D.INTERNODE FROM TMP1 A, TMP2 B, TMP3 C, TMP4 D WHERE A.NODE_PID = B.NODE_PID AND B.NODE_PID = C.NODE_PID AND D.NODE_PID = C.NODE_PID";
	...
	public List<SearchSnapshot> searchDataByNodePids(List<Integer> pids) throws Exception {
		...
		String sql = "WITH TMP1 AS (SELECT NODE_PID, GEOMETRY FROM AD_NODE WHERE NODE_PID IN ("
				+ ids
