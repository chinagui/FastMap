...
public class Operation implements IOperation {
    ...
    private void caleCatchs(Result result) throws Exception {
        ...
        if (this.command.getCatchInfos() != null && this.command.getCatchInfos().size() > 0) {
            ...
            for (int i = 0; i < this.command.getCatchInfos().size(); i++) {
                ...
                LuNode preNode = (LuNode) nodeSelector.loadById(nodePid, true, false);
                // 分离node挂接的link
     ...
     * @throws Exception
     */
    private void departCatchtNode(Result result, int nodePid, int catchNodePid, LuNode preNode, List<LuLink> links) throws Exception {
        LuNodeSelector nodeSelector = new LuNodeSelector(conn);
        // 用分离挂接的Node替换修行Link对应的几何,以保持精度
        LuNode catchNode = (LuNode) nodeSelector.loadById(catchNodePid, true, true);
        // 获取挂接Node的几乎额
     ...
     * @throws Exception
     */
    private void departCatchBreakLink(double lon, double lat, LuNode preNode, int linkPid, List<LuLink> links, Result result) throws Exception {
        ...
        // 调用打断的API
        com.navinfo.dataservice.engine.edit.operation.topo.breakin.breaklupoint.Command breakCommand = new com.navinfo.dataservice.engine.edit.operation.topo.breakin.breaklupoint.Command(breakJson, breakJson.toString());
        com.navinfo.dataservice.engine.edit.operation.topo.breakin.breaklupoint.Process breakProcess = new com.navinfo.dataservice.engine.edit.operation.topo.breakin.breaklupoint.Process(breakCommand, result, conn);
     ...
     * @throws Exception
     */
    private void moveNodeGeo(LuNode node, double lon, double lat, Result result) throws Exception {
        ...
        // 组装更新node的参数
        // 保证是同一个连接
        com.navinfo.dataservice.engine.edit.operation.obj.lunode.update.Command updatecommand = new com.navinfo.dataservice.engine.edit.operation.obj.lunode.update.Command(updateContent, command.getRequester(), node);
        com.navinfo.dataservice.engine.edit.operation.obj.lunode.update.Process process = new com.navinfo.dataservice.engine.edit.operation.obj.lunode.update.Process(updatecommand, result, conn);
        process.innerRun();
    ...
    private void updateLink(Result result) throws Exception {
        ...
        if (meshes.size() == 1) {
            ...
            if (isChanged) {
                ...
                luLink.setGeometry(GeoTranslator.transform(command.getLinkGeom(), 100000, 0));
            }
        ...
        if (command.getFaces() != null && command.getFaces().size() > 0) {
            for (LuFace face : command.getFaces()) {
                ...
                if (flag) {
                    ...
                    com.navinfo.dataservice.engine.edit.operation.obj.luface.create.Operation opFace = new com.navinfo.dataservice.engine.edit.operation.obj.luface.create.Operation(conn, result);
                    List<IObj> objs = new ArrayList<IObj>();
                    objs.addAll(links);
                    opFace.createFaceByLink(objs, face);
     ...
     * @throws Exception
     */
    private void updataRelationObj(List<LuLink> links, Result result) throws Exception {
