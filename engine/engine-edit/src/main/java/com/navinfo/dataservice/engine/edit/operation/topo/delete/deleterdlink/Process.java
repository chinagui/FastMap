...
public class Process extends AbstractProcess<Command> {
	...
	private Map<String, List<Integer>> confirmRelationObj() throws Exception {
		...
		List<RdTrafficsignal> trafficsignals = trafficsignalSelector
				.loadByLinkPid(true, this.getCommand().getLinkPid());
		...
		RdElectroniceyeSelector rdElectroniceyeSelector = new RdElectroniceyeSelector(
				this.getConn());
		List<RdElectroniceye> eleceyes = rdElectroniceyeSelector
		...
		List<RdGate> rdGate = rdGateSelector.loadByLink(this.getCommand()
				.getLinkPid(), true);
		...
		List<RdSe> rdSes = rdSeSelector.loadRdSesWithLinkPid(this.getCommand()
				.getLinkPid(), true);
		infectList = new ArrayList<Integer>();
		...
		RdSpeedbumpSelector rdSpeedbumpSelector = new RdSpeedbumpSelector(
				this.getConn());
		List<RdSpeedbump> rdSpeedbumps = rdSpeedbumpSelector.loadByLinkPid(this
				.getCommand().getLinkPid(), true);
		infectList = new ArrayList<Integer>();
		...
		infectList = interSelector.loadInterPidByNodePid(link.getsNodePid()
				+ "," + link.geteNodePid(), false);
		...
		RdTollgateSelector rdTollgateSelector = new RdTollgateSelector(
				this.getConn());
		List<RdTollgate> rdTollgates = rdTollgateSelector
