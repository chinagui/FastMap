...
CREATE OR REPLACE PACKAGE BODY pk_merge_pa IS
	...
	BEGIN
		...
		THEN
			Vs := Vs || '<GUIDE_LINK_PID/><GUIDE_LINK_SIDE/><GUIDE_NAME_GROUPID/><X_GUIDE/><Y_GUIDE/>';
		END IF;
		RETURN Vs;
	END;
	...
	BEGIN
		...
		Trace('生成主表update:after');
			...
			SELECT A.Pid,
						 (CASE
								WHEN V_Merge_Geo = 'T' AND A.Geo_Oprstatus  in(0,1) AND Instr(A.Log, '改REL') > 0 THEN
								 A.Geometry
								ELSE
								 B.Geometry
							END) AS Geometry,
						 (CASE
								WHEN V_Merge_Geo = 'T' AND A.Geo_Oprstatus  in(0,1) AND  (Instr(A.Log, '改GUIDEX') > 0 OR Instr(A.Log, '改GUIDEY') > 0)  THEN
								 A.X_Guide
								ELSE
								 B.X_Guide
							END) AS X_Guide,
					...
					--	 (CASE
						--		WHEN V_Merge_Geo = 'T' AND A.Geo_Oprstatus  in(0,1) AND Instr(A.Log, '改位移') > 0 THEN
	...
	BEGIN
		...
		THEN
			...
			INSERT INTO Temp_Mpa_Unchanged --过滤已作业的数据
				SELECT *
					FROM Temp_Pa
				  WHERE Geo_Oprstatus in (0,1)
							 AND Att_Oprstatus in( 0,1);
			Trace(SQL%ROWCOUNT);
...
*/
    ...
    INSERT INTO temp_mpa_log_state
      (pid, log, state)
      SELECT pid,
             pk_merge_pa.merge_log(listagg(t.log, ',') within
                                   GROUP(ORDER BY imp_date)),
             pk_merge_pa.merge_state(listagg(t.state, ',') within
                                     GROUP(ORDER BY imp_date))
        FROM (SELECT t.*
                FROM au_ix_pointaddress t,
                     (SELECT ROWID,
                             pid,
                             COUNT(*) over(PARTITION BY pid ORDER BY 1) total
                        FROM au_ix_pointaddress a
                       WHERE (a.att_oprstatus IN (0, 1) OR
                             a.geo_oprstatus IN (0, 1))) t2
               WHERE t.rowid = t2.rowid
                 AND t2.total > 1
               ORDER BY t.pid, t.imp_date) t
       GROUP BY t.pid;
