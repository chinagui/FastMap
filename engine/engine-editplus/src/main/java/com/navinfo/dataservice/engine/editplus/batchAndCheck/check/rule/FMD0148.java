...
/**
 * 	FM-D01-48		官方原始英文格式检查		DHM	
	检查条件：非删除POI对象
		检查原则：官方原始英文名称中若存在以下，则报出：
	1） 回车符检查：包含回车符的记录；
	2） Tab符检查：包含Tab符号的记录；
	3） 多个空格检查：两个及两个以上空格的记录；
	4） 前后空格检查：名称开始前或者结尾处包含空格的记录；
	报log：英文名称（names-langCode="ENG"）格式：英文名称中不能名存在“xx” （提示信息中的符号全部用中文名称）
	检查名称：官方原始英文名称（type={2}，class=1，langCode=ENG）
 * @author sunjiawei
 */
public class FMD0148 extends BasicCheckRule {
