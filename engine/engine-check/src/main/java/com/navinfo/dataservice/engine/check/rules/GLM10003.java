...
public class GLM10003 extends baseRule {
		...
		for (IRow obj : checkCommand.getGlmList()) {
			...
			if (obj instanceof RdSlope) {
				...
				if (outLink.getDirect() == 2
						&& outLink.geteNodePid() == startNode) {
					this.setCheckResult("", "", 0);
					return;
				} else if (outLink.getDirect() == 3
						&& outLink.getsNodePid() == startNode) {
					this.setCheckResult("", "", 0);
					return;
				}
				// 接续线是否沿通行方向联通
			...
			if (obj instanceof RdLink) {
				...
				if (rdLink.changedFields() != null) {
					if (rdLink.changedFields().containsKey("direct")) {
						int direct = (int) rdLink.changedFields().get("direct");
						RdSlopeSelector slopeSelector = new RdSlopeSelector(
								this.getConn());
						List<RdSlope> slopes = slopeSelector.loadByOutLink(
								rdLink.getPid(), false);
	...
	private boolean rdSlopViasConnectedInOneDirection(int startNode,
			List<IRow> viaLinks) throws Exception {
		// 获取接续link的顺序pid
