 ...
 * @author songdongyan
 * @date 2017年1月5日
 * @Description: 如果CRFI中的Link存在I属性，报log
 * 修改CRFI:RdInterLink
 * 新增CRFI:RdInter
 * 道路属性编辑:RdLink
 */
public class GLM28006 extends baseRule{
	...
	private void checkRdLink(RdLink rdLink) throws Exception {
		...
		if(rdLink.changedFields().containsKey("imiCode")){
			...
			if(imiCode==3){
				...
				log.info("RdLink后检查GLM28006:" + sql);
	...
	private void checkRdInterLink(RdInterLink rdInterLink) throws Exception {
		...
		if(rdInterLink.status().equals(ObjStatus.INSERT)){
			...
			String sql = sb.toString();
			log.info("RdInterLink后检查GLM28006:" + sql);
