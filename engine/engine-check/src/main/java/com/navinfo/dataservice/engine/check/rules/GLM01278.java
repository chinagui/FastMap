...
public class GLM01278 extends baseRule {
    ...
    public void postCheck(CheckCommand checkCommand) throws Exception {
        ...
        for (Integer linkPid : check1) {
            ...
            sb.append("SELECT RL.GEOMETRY, '[RD_LINK,' || RL.LINK_PID || ']' TARGET, RL.MESH_ID FROM RD_LINK RL, " +
                    "RD_LINK_LIMIT RLL WHERE RL.LINK_PID = RLL.LINK_PID AND RL.LINK_PID = ");

            sb.append(linkPid);

            sb.append(" AND RL.DIRECT <> 1 AND RLL.TYPE = 6 AND RL.U_RECORD <> 2 AND RLL.U_RECORD <> 2 AND RLL" +
                    ".LIMIT_DIR <> RL.DIRECT   ");
