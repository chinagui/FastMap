...
public class Operation {
	...
	public String updownDepart(List<RdLink> links,
			Map<Integer, RdLink> leftLinks, Map<Integer, RdLink> rightLinks,
			Map<Integer, RdLink> notargetLinkMap, Result result)
			throws Exception {
		...
		List<Integer> nodePids = Arrays.asList(
				tmpNodePids.toArray(new Integer[] {})).subList(1,
				tmpNodePids.size() - 1);
		...
		for (RdWarninginfo info : list) {
			...
			Object sNodePid = null;
			Object eNodePid = null;
			if (link.changedFields().containsKey("sNodePid")) {
				sNodePid = link.changedFields().get("sNodePid");
			}
			if (link.changedFields().containsKey("eNodePid")) {
				eNodePid = link.changedFields().get("eNodePid");
			}
