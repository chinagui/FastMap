...
public class Operation implements IOperation {
	...
	private void caleCatchs(Result result) throws Exception {
		if (this.command.getCatchInfos() != null
			...
			for (int i = 0; i < this.command.getCatchInfos().size(); i++) {
				...
				ZoneNode preNode = (ZoneNode) nodeSelector.loadById(nodePid,
						true, true);
				// 分离node挂接的link
				List<ZoneLink> links = linkSelector
	...
	private void departCatchtNode(Result result, int nodePid, int catchNodePid,
			ZoneNode preNode, List<ZoneLink> links) throws Exception {
		ZoneNodeSelector nodeSelector = new ZoneNodeSelector(conn);
		// 用分离挂接的Node替换修行Link对应的几何,以保持精度
		ZoneNode catchNode = (ZoneNode) nodeSelector.loadById(catchNodePid,
				true, true);
		// 获取挂接Node的几乎额
	 ...
	 * @throws Exception
	 */
	private void departCatchBreakLink(double lon, double lat, ZoneNode preNode,
			int linkPid, List<ZoneLink> links, Result result) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void moveNodeGeo(ZoneNode node, double lon, double lat,
	...
	private void updateLink(Result result) throws Exception {
		...
		if (meshes.size() == 1) {
			...
			if (isChanged) {
				...
				link.setGeometry(GeoTranslator.transform(command.getLinkGeom(),
						100000, 0));
			}
				...
				if (geomInter instanceof GeometryCollection) {
					...
					for (int i = 0; i < geoNum; i++) {
						...
						if (subGeo instanceof LineString) {
							...
							links.addAll(ZoneLinkOperateUtils
									.getCreateZoneLinksWithMesh(subGeo, maps,
											result,
					...
					links.addAll(ZoneLinkOperateUtils
							.getCreateZoneLinksWithMesh(geomInter, maps,
