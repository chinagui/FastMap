...
public class CompLineUtil {
    ...
    public static DoubleLine[] offset(DoubleLine line,double distance,boolean isTwoWay){
        ...
        double degreeDistX = GeometryUtils.convertX2Degree(distance, s.getY());
        double degreeDistY = GeometryUtils.convert2Degree(distance);
        double deltaX = 0;
        double deltaY = 0;
        if(s.getX()==e.getX()){//垂直线，offset只要平移X轴坐标
            if(e.getY()>s.getY()){
                deltaX = -degreeDistX;// PI/2
            }else{
                deltaX = degreeDistX;// PI*3/2
            }
        }else if(s.getY()==e.getY()){//水平线
            if(e.getX()>s.getX()){
                deltaY = degreeDistY;// 0
            }else{
                deltaY = -degreeDistY;// PI
            }
        }else{
            /**
             ...
             * */
            deltaX = degreeDistX*(-(e.getY()-s.getY())/lineEucLength);
            deltaY = degreeDistY*(e.getX()-s.getX())/lineEucLength;
        }
