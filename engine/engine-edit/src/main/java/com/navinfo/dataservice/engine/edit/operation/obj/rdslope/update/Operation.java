...
public class Operation implements IOperation {
    ...
    private void updateRdSlope(Result result) throws Exception {
        ...
        if (content.containsKey("objStatus")) {
            boolean isChanged = this.command.getSlope().fillChangeFields(content);

            if (isChanged) {
                result.insertObject(this.command.getSlope(), ObjStatus.UPDATE, this.command.getSlope().getPid());
            }
    ...
    private void handRdSlopeVia(Result result) throws Exception {
        ...
        if (this.command.getOutLinkPid() != 0 && this.command.getOutLinkPid() != this.command.getSlope().getLinkPid()) {
            for (IRow row : this.command.getSlope().getSlopeVias()) {
                result.insertObject(row, ObjStatus.DELETE, this.command.getPid());
            }
            // 调用特殊打断
            ...
            if (sourceSize > currentSize) {
                for (int i = currentSize; i < sourceSize; i++) {
                    result.insertObject(this.command.getSlope().getSlopeVias().get(i), ObjStatus.DELETE, this.command.getPid());
    ...
    private void breakRelation(Result result) throws Exception {
        ...
        com.navinfo.dataservice.engine.edit.operation.obj.rdslope.create.Command command = new com.navinfo.dataservice.engine.edit.operation.obj.rdslope.create.Command(breakJson, this.command.getRequester());
        ...
        if (link != null) {
            this.command.getSeriesLinkPids().set(this.command.getSeriesLinkPids().size() - 1, link.getPid());
        }
    ...
    public void breakRdLink(Map<RdNode, List<RdLink>> nodeLinkRelation, int linkPid, List<RdLink> newLinks, Result result) throws Exception {
        ...
        for (RdSlope slope : slopes) {
            ...
            for (RdLink link : newLinks) {
                if (slope.getNodePid() == link.getsNodePid() || slope.getNodePid() == link.geteNodePid()) {
                    preLink = link;
                    break;
                }
            }
            ...
            for (int i = 0; i < slinks.size(); i++) {
                if (i == 0) {
                    slope.changedFields().put("linkPid", slinks.get(i).getPid());
                    result.insertObject(slope, ObjStatus.UPDATE, slope.getPid());
                    continue;
                }
                this.addRdslopeVia(slope.getPid(), slinks.get(i).getPid(), i, result);
        ...
        for (RdSlopeVia via : rdSlopeVias) {
            RdLink preLink = selector.loadBySeriesRelationLink(via.getSlopePid(), via.getSeqNum() - 1, true);
            RdSlope slope = (RdSlope) selector.loadById(via.getSlopePid(), true);
            List<RdLink> links = new ArrayList<RdLink>();

            if (preLink == null) {
                preLink = selector.loadByOutLinkBySlopePid(via.getSlopePid(), true);
            }
            ...
            for (int i = 0; i < links.size(); i++) {
                if (i == 0) {
                    ...
                    result.insertObject(via, ObjStatus.UPDATE, via.getSlopePid());
                    continue;
                }
                this.addRdslopeVia(via.getSlopePid(), links.get(i).getPid(), via.getSeqNum() + i, result);
            }
            ...
            if (links.size() == 0) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (v.getSeqNum() >= via.getSeqNum()) {
                        result.insertObject(v, ObjStatus.DELETE, v.getSlopePid());
                        iterator.remove();
                    }
                }
            }
     ...
     * @param result
     */
    private void addRdslopeVia(int slopePid, int linkPid, int seqNum, Result result) {
     ...
     * @param result
     */
    private void caleRdSlopeVia(RdSlope slope, int seqNum, int size, Result result) {
        for (IRow row : slope.getSlopeVias()) {
            RdSlopeVia rdSlopeVia = (RdSlopeVia) row;
            if (rdSlopeVia.getSeqNum() > seqNum) {
                rdSlopeVia.changedFields().put("seqNum", rdSlopeVia.getSeqNum() + size - 1);
                result.insertObject(rdSlopeVia, ObjStatus.UPDATE, rdSlopeVia.getSlopePid());
            }
        }
    }
     ...
     * 获取和退出线联通link组
     *
     * @param resultLinks 新组成挂接link
     * @param sourceLinks 原挂接link组
     * @param preLink
     */
    private void caleRelationRdLink(List<RdLink> resultLinks, List<RdLink> sourceLinks, RdLink preLink) {
        if (resultLinks.size() < sourceLinks.size()) {
            for (RdLink link : sourceLinks) {
                if (resultLinks.contains(link)) {
                    continue;
                }
                if (preLink.getsNodePid() == link.getsNodePid() || preLink.getsNodePid() == link.geteNodePid() || preLink.geteNodePid() == link.getsNodePid() || preLink.geteNodePid() == link.geteNodePid()) {
    ...
    public void departNode(RdLink link, int nodePid, List<RdLink> rdlinks, Result result) throws Exception {
        ...
        for (RdSlope slope : slopes) {
            ...
            RdLink preLink = null;
            if (currSeqNum == 1) {
                preLink = (RdLink) RdLinkSelector.loadById(slope.getLinkPid(), true, true);
            } else {
                for (IRow Row : slope.getSlopeVias()) {
                    RdSlopeVia via = (RdSlopeVia) Row;
