...
import com.navinfo.dataservice.FosEngine.edit.display.AdLinkTileBuilder.TileMapper;
import com.navinfo.dataservice.FosEngine.edit.display.AdLinkTileBuilder.TilePartitioner;
import com.navinfo.dataservice.FosEngine.edit.display.AdLinkTileBuilder.TileReducer;
 ...
 * 构建瓦片类
 */
public class RwLinkTileBuilder {
	...
	public static class TileMapper extends
		...
		public void setup(Context context) {
			...
			taskNum = Integer.valueOf(conf.get("adlinkTaskNum"));
		}
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			String sql = "select a.link_pid, a.color, b.name, sdo_util.to_wktgeometry(a.geometry) geometry   from rw_link a,        (select          b.link_pid, c.name           from rw_link_name b, rd_name c          where b.name_groupid = c.name_groupid            and c.lang_code = 'CHI'            and b.u_record != 2) b  where a.u_record != 2    and a.link_pid = b.link_pid(+) and mod(a.link_pid,"+taskNum+")="
						+ mod;
		...
		public void writeLine(String wktLine, int linkPid, Context context,
				Text keyText, Text valueText, byte degree, String name,
			...
			while (it.hasNext()) {
				...
				if (interWkt.startsWith("LINESTRING")) {
					...
					jsonM.put("a", name != null ? name : JSONNull.getInstance());

					jsonM.put("b",
							color != null ? color : JSONNull.getInstance());
	...
	public static void run(Configuration conf,String tabPrefix) throws Exception{

		String tabName = (StringUtils.isEmpty(tabPrefix)?"":tabPrefix+"_")+"RWLINK_" + conf.get("dbId");
		...
		int numTask = Integer.valueOf(conf.get("rwlinkTaskNum"));
