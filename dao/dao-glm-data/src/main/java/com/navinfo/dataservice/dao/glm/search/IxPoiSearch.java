...
public class IxPoiSearch implements ISearch {
	...
	public List<SearchSnapshot> searchDataByTileWithGap(int x, int y, int z,
		...
		sb.append("WITH TMP1 AS"
				...
				+ "         I.ROW_ID"
				+ "    FROM IX_POI I"
				+ "   WHERE SDO_RELATE(I.GEOMETRY,"
				+ "                    SDO_GEOMETRY(:1,8307),"
				+ "                    'MASK=ANYINTERACT') = 'TRUE'"
				+ "     AND I.U_RECORD != 2),"
				+ " TMP2 AS"
				+ " (SELECT /*+ NO_MERGE(A),INDEX(PN) */"
				+ "   PN.NAME, PN.POI_PID PID"
				+ "    FROM TMP1 A, IX_POI_NAME PN"
				+ "   WHERE PN.POI_PID = A.PID"
				+ "     AND PN.LANG_CODE = 'CHI'"
				+ "     AND PN.NAME_CLASS = 1"
				+ "     AND PN.NAME_TYPE = 2"
				+ "     AND PN.U_RECORD != 2),"
				+ " TMP3 AS"
				+ " (SELECT /*+ NO_MERGE(A),INDEX(P) */"
				+ "   P.PID, P.IS_PLAN_SELECTED, P.IS_IMPORTANT"
				+ "    FROM TMP1 A, DATA_PLAN P"
				+ "   WHERE P.DATA_TYPE = 1"
				+ "     AND P.TASK_ID = :2"
				+ "     AND A.PID = P.PID),"
				+ " TMP4 AS"
				+ " (SELECT /*+ NO_MERGE(A),INDEX(B) */B.PID, B.STATUS, B.QUICK_SUBTASK_ID, B.MEDIUM_SUBTASK_ID"
				+ "    FROM TMP1 A, POI_EDIT_STATUS B"
				+ "   WHERE A.PID = B.PID)"
				+ " SELECT /*+NO_MERGE(B)*/B.*,"
				+ "       C . NAME,"
				+ "       A.IS_PLAN_SELECTED,"
				...
				+ "       D.QUICK_SUBTASK_ID,"
				+ "       D.MEDIUM_SUBTASK_ID"
				+ "  FROM TMP3 A, TMP1 B, TMP2 C, TMP4 D"
				+ " WHERE B.PID = A.PID"
				+ "   AND B.PID = C.PID(+)"
				+ "   AND B.PID = D.PID(+)");
		PreparedStatement pstmt = null;
