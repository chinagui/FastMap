  ...
  for a in (select table_name
             ...
             where exists (select null
                     ...
                     where a.table_name = b.TABLE_NAME
                       and b.COLUMN_NAME = 'ROW_ID')) loop
      execute immediate 'update ' || a.table_name ||
                        ' set row_id = sys_guid()';
...
-- rd_name
drop database link DBLINK_RMS;

create database link DBLINK_RMS
  connect to &1 identified by &2
  using '(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = &3 )(PORT = &5 )))(CONNECT_DATA = (SERVICE_NAME = &4 )))';

create or replace view rd_name as
select "NAME_ID","NAME_GROUPID","LANG_CODE","NAME","TYPE","BASE","PREFIX","INFIX","SUFFIX","NAME_PHONETIC","TYPE_PHONETIC","BASE_PHONETIC","PREFIX_PHONETIC","INFIX_PHONETIC","SUFFIX_PHONETIC","SRC_FLAG","ROAD_TYPE","ADMIN_ID","CODE_TYPE","VOICE_FILE","SRC_RESUME","PA_REGION_ID","MEMO","ROUTE_ID","U_RECORD","U_FIELDS","SPLIT_FLAG","CITY","PROCESS_FLAG" from RD_NAME@dblink_rms;
