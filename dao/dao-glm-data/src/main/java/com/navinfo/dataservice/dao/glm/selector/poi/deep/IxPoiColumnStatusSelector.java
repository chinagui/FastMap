...
//		sql.append(" order by w.second_work_item");
	...
	public JSONObject getColumnCount(Subtask subtask,long userId) throws Exception{
		...
		try {
			...
			sb.append("      and ((c.first_work_item in ('poi_name', 'poi_address','poi_englishname','poi_englishaddress') and s.first_work_status = 1) or");
			sb.append("          (c.second_work_item in ('deepDetail', 'deepParking', 'deepCarrental')) and s.second_work_status = 1)");
			sb.append("      and s.handler = 0)");
