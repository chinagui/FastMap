...
//		}
	...
	protected int extendLog(Connection conn)throws Exception{
		...
		SqlClause sqlClause = getExtendLogSql(conn);
	...
	/**
	 * 补充改成剔除：跨图幅不能落（在时间范围内，存在履历锁记不在日落月图幅范围内，则该POI不落，即：剔除该poi的所有履历）；
	 * @return 
	 */
	@Override
	protected SqlClause getExtendLogSql(Connection conn) throws Exception{
		StringBuilder sb = new StringBuilder();
		sb.append("DELETE FROM "+tempTable+" L");
		sb.append(" WHERE L.OP_ID IN");
		sb.append("       (SELECT T.OP_ID");
		sb.append("          FROM "+tempTable+" T, LOG_DETAIL D, LOG_DETAIL_GRID g");
		sb.append("         WHERE T.OP_ID = D.OP_ID");
