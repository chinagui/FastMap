...
//			DbUtils.closeQuietly(dDbConn);
	...
	private static void insertDataTotable(Integer dDbID, int taskId, String subts) throws Exception {
		...
		try{
			...
			insertPoiTaskTempDataSql.append( " insert into POI_TASK_TEMP (pid,task_id) select distinct pid, "+taskId+" task  from ("
				+ "  select s.pid   from poi_edit_status s where s.quick_subtask_id  in (select to_number(column_value) from table(clob_to_table(?))) "
				+ " 	union all "
				+ "  select s.pid   from poi_edit_status s where s.medium_subtask_id in (select to_number(column_value) from table(clob_to_table(?))) "
				+ " ) ");
