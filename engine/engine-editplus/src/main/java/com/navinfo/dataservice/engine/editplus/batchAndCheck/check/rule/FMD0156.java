...
/**
 * 	FM-D01-56	官方原始英文名不能以THE/No.结尾；No.的“.”前后不能有空格	DHM	
	检查条件：
	非删除POI对象；
	检查原则：
	1.官方原始英文名字段不能以“半角空格+THE（不区分大小写）”结尾，否则报log：英文名全称和简称都不能以THE结尾！（屏蔽条件：英文名对应的中文名全部由字母数字【ａ-ｚＡ-Ｚ０-９＠＿－／；：～＾“‘’”，．？！＊＃（）＜＞￥＄％＆＋＇＂·　、。《》°】组成的不查）
	2.官方原始英文名字段不能以“半角空格+No.（不区分大小写）”结尾，否则报出LOG：英文名全称和简称都不能以No.结尾！
	3.官方原始英文名字段中含有No.的“.”点的前后不能存在空格，否则报出LOG：英文名中No的.前后存在空格！
	检查名称：官方原始英文名称（type={2}，class=1，langCode=ENG）
 * @author sunjiawei
 */
public class FMD0156 extends BasicCheckRule {
	...
	public void runCheck(BasicObj obj) throws Exception {
		if(obj.objName().equals(ObjectName.IX_POI)){
			...
			for(IxPoiName nameTmp:names){
				if(nameTmp.getNameType()==2&&nameTmp.getNameClass()==1
						&&nameTmp.isEng()){
