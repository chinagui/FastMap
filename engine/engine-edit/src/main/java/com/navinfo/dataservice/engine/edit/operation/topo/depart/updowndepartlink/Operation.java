...
public class Operation implements IOperation {
	...
	private void createDepartLinks(Result result) throws Exception {
		...
		for (int i = 0; i < links.size(); i++) {
			lineStrings[i] = (JtsGeometryFactory.createLineString(GeoTranslator
					.transform(links.get(i).getGeometry(), 0.00001, 5)
					.getCoordinates()));
		}
		...
		Point sPoint = JtsGeometryFactory.createPoint(GeoTranslator.transform(
				this.getStartAndEndNode(links, 0).getGeometry(), 0.00001, 5)
				.getCoordinate());
		...
		for (int i = 0; i < command.getLinkPids().size(); i++) {
			...
			this.command.getRightLinkMapping().put(currentLink.pid(),
					departLink);
		...
		for (int i = lines.length - 1; i >= command.getLinkPids().size(); i--) {
			...
			upLists.addAll(this.createUpRdLink(departLink, result, currentLink,
					nextLink, map));

			this.command.getLeftLinkMapping()
					.put(currentLink.pid(), departLink);
		}
		// 维护挂接线信息
	...
	private void createInnerLine(LineString lineDownString,
		...
		for (RdLink link : links) {
			...
			if (link.changedFields().containsKey("leftRegionId")) {
				link.setLeftRegionId(Integer.valueOf(link.changedFields()
						.get("leftRegionId").toString()));
			}
			if (link.changedFields().containsKey("rightRegionId")) {
				link.setRightRegionId(Integer.valueOf(link.changedFields()
						.get("rightRegionId").toString()));
			}
		}
	 ...
	 * @throws Exception
	 */
	private void updateAdjacentLines(LineString[] lines,
			Map<Geometry, RdNode> map, Result result) throws Exception {
		RdLinkSelector nodeSelector = new RdLinkSelector(conn);
				...
				if (CompPolylineUtil.isRightSide(JtsGeometryFactory
					...
					targetLine = CompPolylineUtil.cut(lines[i], lines[i + 1],
							JtsGeometryFactory.createLineString(GeoTranslator
									...
									.getCoordinates()), currentPoint, true);
				} else {
					targetLine = CompPolylineUtil.cut(lines[lines.length - i
							- 1], lines[lines.length - i - 2],
							JtsGeometryFactory.createLineString(GeoTranslator
									.transform(link.getGeometry(), 0.00001, 5)
									.getCoordinates()), currentPoint, false);
				}
				if (targetLine.getCoordinate() != null) {

					this.updateadjacentLine(targetLine, link, currentPid, map,
							result);
	...
	private void updateadjacentLine(LineString targetLine, RdLink link,
			int currentPid, Map<Geometry, RdNode> map, Result result)
			throws Exception {
		JSONObject updateContent = new JSONObject();
		updateContent.put("geometry", GeoTranslator.jts2Geojson(targetLine));
		link.fillChangeFields(updateContent);
		Point point = null;
		if (map.containsKey(JtsGeometryFactory.createPoint(targetLine
				.getCoordinates()[0]))) {
			point = JtsGeometryFactory
					.createPoint(targetLine.getCoordinates()[0]);
		} else {
			point = JtsGeometryFactory
					.createPoint(targetLine.getCoordinates()[targetLine
							.getCoordinates().length - 1]);
		}
	...
	private List<RdLink> createUpRdLink(RdLink departLink, Result result,
		...
		if (sourceLink.getPid() == sourceNextLink.getPid()) {
			return this.createEndRdLink(departLink, result, sourceLink,
					sourceNextLink, map);
		}
		...
		// 如果至少有一条下挂的link对于上(左)线需要生成新的node
			...
			sNode = this.getDepartRdlinkNode(links, departLink, sourceLink,
					sourceNextLink, 1, 1, map, result);
			eNode = this.getDepartRdlinkNode(links, departLink, sourceLink,
					sourceNextLink, 0, 1, map, result);
		}
	...
	private List<RdLink> createDownRdLink(RdLink departLink, Result result,
		...
		if (links.size() <= 0) {
			...
			sNode = this.getDepartRdlinkNode(links, departLink, sourceLink,
					sourceNextLink, 1, 0, map, result);
			eNode = this.getDepartRdlinkNode(links, departLink, sourceLink,
					sourceNextLink, 0, 0, map, result);
		}
		return RdLinkOperateUtils.addRdLink(sNode, eNode, departLink,
				sourceLink, result);
	}
	...
	private List<RdLink> createEndRdLink(RdLink departLink, Result result,
		...
		RdNode sNode = null;
		RdNode eNode = null;
		sNode = map.get(JtsGeometryFactory.createPoint(departLink.getGeometry()
				.getCoordinates()[0]));
		eNode = map
				.get(JtsGeometryFactory.createPoint(departLink.getGeometry()
	 ...
	 * @throws Exception
	 */
	private RdNode getDepartRdlinkNode(List<RdLink> links, RdLink departLink,
			RdLink sourceLink, RdLink sourceNextLink, int flag, int flagUpDown,
			Map<Geometry, RdNode> map, Result result) throws Exception {
		...
		for (RdLink link : links) {
			flagBooleans
					.add(this.isRightSide(sourceLink, sourceNextLink, link));
		}
	 ...
	 * @throws Exception
	 */
	private RdNode getNodeByDepartGeo(RdLink departLink, int flag,
			Map<Geometry, RdNode> map, Result result) throws Exception {
		RdNode node = null;
		...
		if (flag == 1) {
			...
			coordinate = departLink.getGeometry().getCoordinates()[departLink
		...
		if (map.containsKey(geometry)) {
			...
			map.put(GeoTranslator.transform(node.getGeometry(), 0.00001, 5),
					node);
	...
	private RdNode updateAdNodeForTrack(RdLink link, int nodePid,
			Map<Geometry, RdNode> map, Result result, int flag)
			throws Exception {
		JSONObject updateContent = new JSONObject();
		...
		Geometry geometry = null;
		if (flag == 1) {
			geometry = JtsGeometryFactory.createPoint(link.getGeometry()
					.getCoordinates()[0]);
			if (map.containsKey(geometry)) {
			...
			geometry = JtsGeometryFactory
					.createPoint(link.getGeometry().getCoordinates()[link
							.getGeometry().getCoordinates().length - 1]);
			if (map.containsKey(geometry)) {
				return map.get(geometry);
			}
	...
	private int getIntersectPid(RdLink fristLink, RdLink secondLink) {
		if (fristLink.getsNodePid() == secondLink.getsNodePid()
				|| fristLink.getsNodePid() == secondLink.geteNodePid()) {
	...
	private Point getIntersectPoint(RdLink fristLink, RdLink secondLink) {
		if (fristLink.getsNodePid() == secondLink.getsNodePid()
				|| fristLink.getsNodePid() == secondLink.geteNodePid()) {
			return JtsGeometryFactory.createPoint(GeoTranslator.transform(
					fristLink.getGeometry(), 0.00001, 5).getCoordinates()[0]);
		} else {
			return JtsGeometryFactory
					.createPoint(GeoTranslator.transform(
							fristLink.getGeometry(), 0.00001, 5)
							.getCoordinates()[GeoTranslator.transform(
							fristLink.getGeometry(), 0.00001, 5)
							.getCoordinates().length - 1]);
		}
