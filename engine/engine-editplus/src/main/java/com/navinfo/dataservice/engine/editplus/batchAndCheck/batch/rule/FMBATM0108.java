 ...
 * 【批处理执行时间】POI数据日落月完成POI精编前批之前
	...
	【批处理字段】IX_POI表中x_guide、y_guide、link_pid、name_groupid、side、pmesh_id
	【批处理原则】
	根据引导坐标3m范围找Link，如果存在，则进行批处理，如果找到多条，则取最近的一条进行批处理；
	如果不存在，则需要把引导link赋值为0。
	字段赋值原则如下：
	引导坐标：求出最近点保留5位后与原来一致则不修改；如果不一致，则取最近点保留5位精度。
	SIDE：基于显示坐标计算显示坐标与link的距离，如果在1.5米之内，则为“3 link上”；如果在1.5米之外，则根据link的画线方向计算，如果在划线方向的左侧，则side赋值为“1 Link左侧”，如果在划线方向右侧，则赋值为“2 Link右侧”；
	PMESH_ID：取关联link的mesh_id
	NAME_GROUPID:赋值为0
 * 
 * @author gaopengrong
 *
