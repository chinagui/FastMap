...
public class Day2MonthPoiMerge915TmpJob extends AbstractJob {
			...
			try {
				for(Region region:regions){
					...
					if(!tmpOpTable.isEmpty()){
						doMediumSync(region,null,null,null,datahubApi,d2mSyncApi,manApi,tmpOpTable);
					}else{
						List<Integer> filterGrids = new ArrayList<Integer>();
						List<Integer> logGrids =selectLogGridsByTaskId(null,datahubApi,region,0);//查询所有存在可落履历的grids
						...
						doMediumSync(region,filterGrids,null,null,datahubApi,d2mSyncApi,manApi,null);
					}
				}
	...
	private void doMediumSync(Region region,List<Integer> filterGrids,List<Integer> grids,List<Integer> taskIds,DatahubApi datahubApi, Day2MonthSyncApi d2mSyncApi,ManApi manApi,String tmpOpTable)
			throws Exception{
