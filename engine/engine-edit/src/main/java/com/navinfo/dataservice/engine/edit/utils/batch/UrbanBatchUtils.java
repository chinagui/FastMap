...
public class UrbanBatchUtils extends BaseBatchUtils {
	...
	private static LuFaceSelector luFaceSelector = null;

	private static LuFaceSelector getLuFaceSelector(Connection conn) throws Exception {
		if (null == luFaceSelector)
			luFaceSelector = new LuFaceSelector(conn);
		return luFaceSelector;
	}
	...
	public static void updateUrban(RdLink link, Connection conn, Result result) throws Exception {
		// 将link几何缩小100000倍，根据link几何查找与之相关的BUA面
		Geometry linkGeometry = shrink(link.getGeometry());
		List<LuFace> faces = getLuFaceSelector(conn).loadRelateFaceByGeometry(linkGeometry);
		// 如关联面数量为空或大于一暂不做处理
