...
//		   AND N.LANG_CODE = 'CHI'
//		   AND N.NAME LIKE '%隧道%'
		String sql1 = "SELECT DISTINCT N.NAME_ID, N.TYPE FROM RD_LINK R, RD_LINK_FORM F, RD_NAME N WHERE R.LINK_PID = :1 AND F.LINK_PID = R.LINK_PID AND F.FORM_OF_WAY = 31 AND N.NAME_GROUPID IN (SELECT DISTINCT LN.NAME_GROUPID FROM RD_LINK_NAME LN WHERE LN.LINK_PID = R.LINK_PID AND LN.NAME_CLASS = 1) AND N.LANG_CODE = 'CHI' AND N.NAME LIKE '%隧道%'";
...
//		   AND LN.NAME_CLASS = 1
//		   AND LN.NAME_TYPE = 5
		String sql6 = "SELECT DISTINCT LN.NAME_GROUPID FROM RD_LINK R, RD_LINK_FORM F, RD_LINK_NAME LN WHERE R.LINK_PID = :1 AND F.LINK_PID = R.LINK_PID AND F.FORM_OF_WAY = 31 AND LN.LINK_PID = R.LINK_PID AND LN.NAME_CLASS = 1 AND LN.NAME_TYPE = 5";
...
//		   )
		String sql4 = "SELECT N.NAME, N.TYPE FROM RD_LINK R, RD_LINK_FORM F, RD_NAME N, RD_LINK_NAME LN WHERE F.LINK_PID = R.LINK_PID AND F.FORM_OF_WAY = 31 AND LN.LINK_PID = R.LINK_PID AND LN.NAME_CLASS = 1 AND N.NAME_GROUPID = LN.NAME_GROUPID AND N.LANG_CODE = 'CHI' AND R.LINK_PID IN (SELECT RL.LINK_PID FROM RD_LINK RL WHERE RL.S_NODE_PID IN (:1,:2) OR RL.E_NODE_PID IN (:3,:4)) MINUS SELECT N.NAME, N.TYPE FROM RD_LINK R, RD_LINK_FORM F, RD_NAME N, RD_LINK_NAME LN WHERE F.LINK_PID = R.LINK_PID AND F.FORM_OF_WAY = 31 AND LN.LINK_PID = R.LINK_PID AND LN.NAME_CLASS = 1 AND N.NAME_GROUPID = LN.NAME_GROUPID AND N.LANG_CODE = 'CHI' AND R.LINK_PID IN (SELECT RL.LINK_PID FROM RD_LINK RL WHERE RL.LINK_PID != :5 AND (RL.S_NODE_PID IN (:6,:7) OR RL.E_NODE_PID IN (:8,:9)))";
