	...
	private void endJob(int status,String resultMsg){
		...
		if(parent==null){//独立job
			try{
				...
				long durationSeconds = (long) Math.ceil((jobInfo.getEndTime().getTime()-jobInfo.getBeginTime().getTime())/1000.0);
				JobMsgPublisher.endJob(jobInfo.getUserId(),jobInfo.getId(), status,resultMsg,jobInfo.getResponse(),request.getJobTypeName(),durationSeconds);
			}catch(Exception e){
