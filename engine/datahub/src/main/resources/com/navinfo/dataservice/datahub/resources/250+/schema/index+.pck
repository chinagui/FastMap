BEGIN
  FOR A IN (SELECT TABLE_NAME FROM USER_TABLES WHERE TABLE_NAME IN ('NI_VAL_EXCEPTION','CK_EXCEPTION')) LOOP
    BEGIN
      EXECUTE IMMEDIATE 'ALTER TABLE '|| A.TABLE_NAME ||' ADD (U_RECORD NUMBER(2) default 0 not null check (U_RECORD in (0,1,2,3)),U_FIELDS VARCHAR2(1000),U_DATE VARCHAR2(14),ROW_ID RAW(16))';
    EXCEPTION
      WHEN OTHERS THEN
