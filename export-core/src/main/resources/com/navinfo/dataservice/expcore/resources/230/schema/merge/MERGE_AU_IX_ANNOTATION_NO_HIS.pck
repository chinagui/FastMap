...
CREATE OR REPLACE PACKAGE BODY merge_au_ix_annotation_no_his IS
      ...
      SELECT *
       ...
       (SELECT 1
               ...
               WHERE t.PID = ia.pid
                 ...
                 AND instr(t.field_modify_flag ,''新增'')>0 and instr(t.field_modify_flag ,''删除'')=0                
                 AND EXISTS (SELECT 1
    ...
    IF (v_pid_count > 0) THEN
      ...
      SELECT PID_MAN.PID_NEXTVAL(''IX_ANNOTATION_NAME'') as NAME_ID,
       ...
       WHERE anno.pid = n.pid
         ...
         AND instr(anno.field_modify_flag ,''新增'')>0 and instr(anno.field_modify_flag ,''删除'')=0   
         AND EXISTS (SELECT 1
                FROM temp_au_ix_anno_uniq_task tmp
  ...
  BEGIN
    DELETE FROM temp_ix_anot_name_mg; --为生成履历，采用零食表保存新增的数据    
    v_oprstatus_claus := merge_utils.get_oprstatus_clause(v_merge_type,
                                                          'anno');
    --需要申请pid
    v_not := v_param;
    IF ('NOT' != v_not) THEN
  ...
  BEGIN
    ...
    EXECUTE IMMEDIATE 'INSERT INTO ix_annotation
       ...
       WHERE instr(anno.field_modify_flag ,''新增'')>0 and instr(anno.field_modify_flag ,''删除'')=0   
         AND ' || v_oprstatus_claus || '
         AND EXISTS (SELECT 1
                FROM temp_au_ix_anno_uniq_task tmp
               WHERE tmp.pid = anno.pid)
  ...
  BEGIN
    ...
    FOR rec IN (SELECT a.audata_id, a.pid, a.field_modify_flag AS log
                 ...
                 WHERE (instr(a.field_modify_flag ,'改名称')>0 OR instr(a.field_modify_flag ,'改分类')>0)
                   and instr(a.field_modify_flag ,'新增')=0 and instr(a.field_modify_flag ,'删除')=0   
     ...
     WHERE ian.lang_code IN ('CHI', 'CHT')
       ...
       AND EXISTS (SELECT 1
             ...
             WHERE auia.pid = ian.pid               
               AND instr(auia.field_modify_flag ,'改名称')>0
               and instr(auia.field_modify_flag ,'新增')=0 and instr(auia.field_modify_flag ,'删除')=0   
               AND auia.att_oprstatus = 0)
       AND NOT EXISTS (SELECT 1
    ...
    EXECUTE IMMEDIATE 'DELETE FROM IX_ANNOTATION_FLAG ian
     ...
     (SELECT 1
             ...
             WHERE ian.pid = anno.pid
               AND instr(anno.field_modify_flag ,''删除'')>0
               AND ' || v_oprstatus_claus || '  
               AND EXISTS
  ...
  BEGIN
    ...
    FOR rec IN (SELECT a.audata_id, a.pid, a.field_modify_flag AS log
                  FROM au_ix_annotation a
                 WHERE 
                   instr(a.field_modify_flag ,'改位移')>0 
                   and instr(a.field_modify_flag ,'新增')=0 and instr(a.field_modify_flag ,'删除')=0  
  ...
  BEGIN
    ...
    FOR rec IN (SELECT a.audata_id, a.pid, a.field_modify_flag AS log,a.geo_oprstatus,a.att_oprstatus
                  FROM au_ix_annotation a
                 WHERE (instr(a.field_modify_flag ,'改位移')>0 or instr(a.field_modify_flag ,'改名称')>0 or instr(a.field_modify_flag ,'改分类')>0)
                   and instr(a.field_modify_flag ,'新增')=0 and instr(a.field_modify_flag ,'删除')=0  
                   AND (a.geo_oprstatus = 0 or a.att_oprstatus=0)
                   AND EXISTS
                 (SELECT 1 FROM ix_annotation p WHERE a.pid = p.pid)
                   AND NOT EXISTS (SELECT 1
                          FROM temp_au_ix_anno_mul_task tmp
  ...
  BEGIN
    ...
    USING (SELECT anno.pid, anno.geometry,anno.region_id,anno.mesh_id 
            ...
            WHERE (instr(anno.field_modify_flag ,'改位移')>0 )
                   and instr(anno.field_modify_flag ,'新增')=0 and instr(anno.field_modify_flag ,'删除')=0  
              AND anno.geo_oprstatus = 0
              AND anno.pid = l.pid
  ...
  BEGIN
    ...
    USING (SELECT *
            ...
            WHERE instr(auip.field_modify_flag ,'新增')>0 and instr(auip.field_modify_flag ,'删除')=0 
  ...
  BEGIN
    ...
    USING (SELECT *
             FROM au_ix_annotation auip
            WHERE auip.geo_oprstatus = 0
              AND auip.audata_id = :v_audata_id) rs
    ON (ip.pid = rs.pid)
    WHEN MATCHED THEN
      UPDATE SET ip.geometry = rs.geometry,
                 ip.region_id = rs.region_id,
                 ip.mesh_id = rs.mesh_id'
      USING v_audata_id;
  END;
  ...
  BEGIN
    ...
    ON (ixan.pid = rs.pid)
    WHEN MATCHED THEN
      UPDATE SET ixan.geometry = rs.geometry,
                 ixan.region_id = rs.region_id,
                 ixan.mesh_id = rs.mesh_id'
      USING v_data_id;
  END;
  ...
  BEGIN
    ...
    USING (SELECT p2.pid, p2.field_modify_flag
        ...
        WHERE p2.audata_id = tmp.audata_id and (' ||
                      merge_utils.get_oprstatus_clause(v_merge_type, 'p2') || ')) v
     ON (p1.pid = v.pid)
     WHEN MATCHED THEN
