...
public class RdBranchSearch implements ISearch {
	...
	public List<SearchSnapshot> searchDataByTileWithGap(int x, int y, int z,
		...
		String sql = "with tmp1 as  (select link_pid, geometry     from rd_link    where sdo_relate(geometry, sdo_geometry(:1, 8307), 'mask=anyinteract') =          'TRUE'      and u_record != 2), tmp2 as  (  select /*+ index(a) index(t1)*/    a.in_link_pid,    a.node_pid,    t1.branch_type,    listagg(a.branch_pid || '-' || t1.detail_id, ',') within group(order by 1) pids     from rd_branch a, tmp1 b, rd_branch_detail t1    where a.in_link_pid = b.link_pid      and a.branch_pid = t1.branch_pid      and branch_type in (0, 1, 2, 3, 4)      and a.u_record != 2      and t1.u_record != 2    group by a.in_link_pid, a.node_pid, t1.branch_type    UNION    select /*+ index(a) index(t2) */    a.in_link_pid,    a.node_pid,    5 as branch_type,        listagg(a.branch_pid || '-' || t2.row_id, ',') within group(order by 1) pids     from rd_branch a, tmp1 b, rd_branch_realimage t2     where a.in_link_pid = b.link_pid      and a.branch_pid = t2.branch_pid      and a.u_record != 2      and t2.u_record != 2     group by a.in_link_pid, a.node_pid    UNION    select /*+ index(a) index(t3) */    a.in_link_pid,    a.node_pid,    6 as branch_type,        listagg(a.branch_pid || '-' || t3.signboard_id, ',') within group(order by 1) pids     from rd_branch a, tmp1 b, rd_signasreal t3     where a.in_link_pid = b.link_pid      and a.branch_pid = t3.branch_pid      and a.u_record != 2      and t3.u_record != 2     group by a.in_link_pid, a.node_pid     UNION    select /*+ index(a)  index(t4) */    a.in_link_pid,    a.node_pid,    7 as branch_type,        listagg(a.branch_pid || '-' || t4.row_id, ',') within group(order by 1) pids     from rd_branch a, tmp1 b, rd_seriesbranch  t4    where a.in_link_pid = b.link_pid      and a.branch_pid = t4.branch_pid      and a.u_record != 2      and t4.u_record != 2     group by a.in_link_pid, a.node_pid    UNION    select /*+ index(a)  index(t5) */    a.in_link_pid,    a.node_pid,    8 as branch_type,        listagg(a.branch_pid || '-' || t5.schematic_id, ',') within group(order by 1) pids     from rd_branch a, tmp1 b, rd_branch_schematic t5     where a.in_link_pid = b.link_pid      and a.branch_pid = t5.branch_pid      and a.u_record != 2      and t5.u_record != 2     group by a.in_link_pid, a.node_pid     UNION    select /*+ index(a) index(t6) */    a.in_link_pid,    a.node_pid,    9 as branch_type,        listagg(a.branch_pid || '-' || t6.signboard_id, ',') within group(order by 1) pids     from rd_branch a, tmp1 b, rd_signboard t6     where a.in_link_pid = b.link_pid      and a.branch_pid = t6.branch_pid      and a.u_record != 2      and t6.u_record != 2     group by a.in_link_pid, a.node_pid    ),         tmp3 as  (select in_link_pid,          node_pid,          listagg(branch_type || '~' || pids, '^') within group(order by branch_type) a     from tmp2    group by in_link_pid, node_pid) select /*+ index(c) */  a.a, b.geometry link_geom, c.geometry point_geom   from tmp3 a, tmp1 b, rd_node c  where a.in_link_pid = b.link_pid    and a.node_pid = c.node_pid    and c.u_record != 2";
