...
public class HbasePoiInfoTemp {
	private static Logger log = LoggerRepos.getLogger(HbasePoiInfoTemp.class);
	 ...
	 * @throws Exception
	 */
	public void getHBaseDataInfo(String filePath, String outPath)
			throws Exception {
	...
	private void createMapBetweenPoiAndDbId() throws Exception {
		...
		List<RegionMesh> regions = manApi.queryRegionWithMeshes(this
				.getPoiCollectionByMesh().keySet());

		if (regions == null || regions.size() == 0) {
			log.error(String.format("根据图幅%s未查询到所属大区库信息", StringUtils.join(this
					.getPoiCollectionByMesh().keySet(), ",")));
			return;
		}

		// 以图幅为主键的数据，更新为以dbId为主键
		for (Map.Entry<String, List<PoiInfo>> entry : this
				.getPoiCollectionByMesh().entrySet()) {
			for (RegionMesh region : regions) {
				...
				if (poiDataByDailyDbId.containsKey(region.getDailyDbId())) {
					poiDataByDailyDbId.get(region.getDailyDbId()).addAll(
							entry.getValue());
				} else {
					poiDataByDailyDbId.put(region.getDailyDbId(),
							entry.getValue());
				}
			} // for
		} // for
		...
		// 日库
		for (Map.Entry<Integer, List<PoiInfo>> entry : poiDataByDailyDbId
				.entrySet()) {
	 ...
	 * @throws Exception
	 */
	private void createPoiFlagTable(List<PoiInfo> values) throws Exception {
		...
		try {
			...
			List<Integer> findPoi = getFoundPoi(conn, allPoi, "PID", "IX_POI");

			log.info("finding poi exist in ix_poi_flag_method...");
			List<Integer> hasInsertInFlag = getFoundPoi(conn, findPoi,
					"POI_PID", "IX_POI_FLAG_METHOD");
	...
	private List<Integer> getFoundPoi(Connection conn, List<Integer> allPoiPid,
			String pid, String table) throws Exception {
		...
		String existPoi = String
				.format("SELECT %s FROM %s WHERE %s IN (select to_number(column_value) from table(clob_to_table(?)))",
						pid, table, pid);
