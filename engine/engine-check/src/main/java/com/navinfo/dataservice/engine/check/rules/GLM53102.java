 ...
 * 道路方向编辑
 */
public class GLM53102 extends baseRule{
	...
	private void checkRdLink(RdLink rdLink) throws Exception {
		...
		if(checkFlag){
			...
			sb2.append("SELECT COUNT(1) FROM RD_LINK_INT_RTIC R WHERE R.U_RECORD <> 2 AND R.LINK_PID = " + rdLink.getPid());

			String sql2 = sb2.toString();
			log.info("RdLink后检查GLM53102:" + sql2);
	...
	private void checkRdLinkIntRtic(RdLinkIntRtic rdLinkRtic) throws Exception {
		if(rdLinkRtic.status().equals(ObjStatus.INSERT)){
			...
			sb2.append(" AND (SELECT COUNT(1) FROM RD_LINK_INT_RTIC R WHERE R.U_RECORD <> 2 AND R.LINK_PID = L.LINK_PID) > 1");

			String sql2 = sb2.toString();
			log.info("RdLinkIntRtic后检查GLM53102:" + sql2);
