...
public class RdInterSelector extends AbstractSelector {
	...
	public List<RdInter> loadRdInterByOutLinkPid(int linkPid, boolean isLock) throws Exception {
		...
		String sql = "select * from rd_inter where pid in(select pid from rd_inter_link b where b.pid int(select pid from rd_inter_link where u_record !=2 and link_pid = :1 having count(1) = 1) group by b.pid) and u_record !=2";
