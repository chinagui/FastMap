...
/**
 * 
 * 1.可以为空；2.字符串；3.不为空时，不能含有汉字和非法字符。
 *	 Log：地址道路名拼音含有汉字和非法字符“XX”。
 *
 */
public class FMZY20138 extends BasicCheckRule {
	...
	public void runCheck(BasicObj obj) throws Exception {
		...
		for (IxPoiAddress address:addresses) {
			String addStr = address.getRoadnamePhonetic();
			...
			for (int i=0;i<addStr.length();i++) {
				...
				if (!characterMap.containsKey(String.valueOf(character)) || CheckUtil.isChinese(String.valueOf(character))) {
					errList.add("地址道路名拼音含有汉字和非法字符“"+character+"”。");
				}
			}
		}
		if (errList.size()>0) {
