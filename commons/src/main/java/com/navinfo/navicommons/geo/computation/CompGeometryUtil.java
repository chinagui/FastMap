...
public class CompGeometryUtil {
	...
	public static boolean intersectLine(double[] line1, double[] line2) {
		...
		// 若P1P2跨立Q1Q2 ，则矢量 ( P1 - Q1 ) 和( P2 - Q1 )位于矢量( Q2 - Q1 ) 的两侧，即( P1 -
		// Q1 ) × ( Q2 - Q1 ) * ( P2 - Q1 ) × ( Q2 - Q1 ) < 0。上式可改写成( P1 - Q1 )
		// × ( Q2 - Q1 ) * ( Q2 - Q1 ) × ( P2 - Q1 ) > 0。当 ( P1 - Q1 ) × ( Q2 -
		// Q1 ) = 0 时，说明 ( P1 - Q1 ) 和 ( Q2 - Q1 )共线，但是因为已经通过快速排斥试验，所以 P1 一定在线段
		// Q1Q2上；同理，( Q2 - Q1 ) ×(P2 - Q1 ) = 0 说明 P2 一定在线段
		// Q1Q2上。所以判断P1P2跨立Q1Q2的依据是：( P1 - Q1 ) × ( Q2 - Q1 ) * ( Q2 - Q1 ) × (
		// P2 - Q1 ) >= 0。
		// 同理判断Q1Q2跨立P1P2的依据是：( Q1 - P1 ) × ( P2 - P1 ) * ( P2 - P1 ) × ( Q2 -
		// P1 ) >= 0。
		if (((line1[0] - line2[0]) * (line2[3] - line2[1]) - (line2[2] - line2[0])
				* (line1[1] - line2[1]))
				* ((line2[2] - line2[0]) * (line1[3] - line2[1]) - (line1[2] - line2[0])
						* (line2[3] - line2[1])) < 0
				|| ((line2[0] - line1[0]) * (line1[3] - line1[1]) - (line1[2] - line1[0])
						* (line2[1] - line1[1]))
						* ((line1[2] - line1[0]) * (line2[3] - line1[1]) - (line2[2] - line1[0])
								* (line1[3] - line1[1])) < 0)
			return false;
		return true;
	}
	...
	public static boolean intersectRect(double[] rect1, double[] rect2) {
		...
		// ||Math.min(rect1[1], rect2[1])>Math.max(rect1[3], rect2[3])) return
		// false;
		// return true;
		// 采用中心点长度算法
		return (Math.abs((rect1[0] + rect1[2]) - (rect2[0] + rect2[2])) < (rect1[2]
				- rect1[0] + rect2[2] - rect2[0]) && Math
				.abs((rect1[1] + rect1[3]) - (rect2[1] + rect2[3])) < (rect1[3]
				- rect1[1] + rect2[3] - rect1[1]));
	}
