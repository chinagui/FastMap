...
public class TipsSelector {
	...
	public int getTipsDayTotal(String wkt, Set<Integer> collectTaskIds,
		...
		try {
			...
			return (int) indexOracleOperator.querCount(
	 ...
	 */
	public List<JSONObject> getTipsByTaskId(int taskId, int taskType)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	public SolrDocumentList getTipsByTaskIdAndStatus(int taskId, int taskType)
			throws Exception {
	...
	public Set<Integer> getGridsListByTask(int collectTaskid, int q_TASK_TYPE)
		...
		List<JSONObject> tipsList = conn.queryTipsByTask(collectTaskid,
				q_TASK_TYPE);
	...
	public JSONObject listInfoTipsByPage(String parameter) throws Exception {
		...
		SolrDocumentList sdList = conn.queryTipsSolrDocByPage(queryTotal, null,
				firstNum, pageSize);
		JSONObject jsonObject = new JSONObject();
		long totalNum = sdList.getNumFound();
		if (totalNum <= Integer.MAX_VALUE) {
