...
/**
 * @ClassName: GLM01028_5
 * @author zhangxiaolong
 * @date 2017年2月7日
 * @Description: 10级路/步行街/人渡不能是警示信息的进入线
 */
public class GLM01028_5 extends baseRule {
	...
	public void postCheck(CheckCommand checkCommand) throws Exception {
		...
		for (int linkPid : linkPidSet) {
			logger.debug("检查类型：postCheck， 检查规则：GLM01028_5， 检查要素：RDLINK(" + linkPid + "), 触法时机：LINK种别编辑");
			...
			sb.append(
					" ) SELECT T.GEOMETRY, T.TARGET, T.MESH_ID FROM T WHERE EXISTS (SELECT 1 FROM RD_WARNINGINFO RT WHERE T.LINK_PID = RT.LINK_PID AND RT.U_RECORD <> 2)");
			
			log.info("RdLink后检查GLM01028_5 SQL:" + sb.toString());
