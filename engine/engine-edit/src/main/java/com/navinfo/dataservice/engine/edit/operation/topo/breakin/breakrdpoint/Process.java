...
public class Process extends AbstractProcess<Command> {
    ...
    public boolean prepareData() throws Exception {
        ...
        try {
            ...
            // 获取此LINK上交限进入线
            List<RdRestriction> restrictions = new RdRestrictionSelector(this.getConn()).loadRdRestrictionByLinkPid(this.getCommand()
                    .getLinkPid(), true);

            this.getCommand().setRestrictions(restrictions);

            // 获取此LINK上交限退出线
            List<RdRestrictionDetail> details = new RdRestrictionDetailSelector(this.getConn()).loadDetailsByLinkPid(this.getCommand()
                    .getLinkPid(), true);
            ...
            // 获取此LINK上车信进入线
            List<RdLaneConnexity> laneConnexitys = new RdLaneConnexitySelector(this.getConn()).loadRdLaneConnexityByLinkPid(this
                    .getCommand().getLinkPid(), true);

            this.getCommand().setLaneConnexitys(laneConnexitys);

            // 获取此LINK上车信退出线
            List<RdLaneTopology> topos = new RdLaneTopologySelector(this.getConn()).loadToposByLinkPid(this.getCommand().getLinkPid(),
                    true);

            this.getCommand().setLaneTopologys(topos);

            // 获取LINK上车信经过线
            List<List<Entry<Integer, RdLaneVia>>> laneVias = new RdLaneViaSelector(this.getConn()).loadRdLaneViaByLinkPid(this
                    .getCommand().getLinkPid(), true);

            this.getCommand().setLaneVias(laneVias);

            // 获取link上的点限速
            List<RdSpeedlimit> limits = new RdSpeedlimitSelector(this.getConn()).loadSpeedlimitByLinkPid(this.getCommand().getLinkPid(),
                    true);
            ...
            List<RdBranch> inBranchs = new RdBranchSelector(this.getConn()).loadRdBranchByInLinkPid(this.getCommand().getLinkPid(), true);

            this.getCommand().setInBranchs(inBranchs);

            // 获取已该LINK作为分歧退出线的分歧

            List<RdBranch> outBranchs = new RdBranchSelector(this.getConn()).loadRdBranchByOutLinkPid(this.getCommand().getLinkPid(),
                    true);
            ...
            List<List<RdBranchVia>> branchVias = new RdBranchViaSelector(this.getConn()).loadRdBranchViaByLinkPid(this.getCommand()
                    .getLinkPid(), true);
            ...
            List<RdTollgate> tollgates = tollgateSelector.loadRdTollgatesWithLinkPid(this.getCommand().getLinkPid(), true);
            this.getCommand().setRdTollgates(tollgates);

            // 获取该link关联的限高限重
            RdHgwgLimitSelector hgwgLimitSelector = new RdHgwgLimitSelector(this.getConn());
            List<RdHgwgLimit> hgwgLimits = hgwgLimitSelector.loadByLinkPid(this.getCommand().getLinkPid(), true);
            this.getCommand().setRdHgwgLimits(hgwgLimits);

            // 获取该link关联的里程桩
            RdMileagepileSelector mileagepileSelector = new RdMileagepileSelector(this.getConn());
            List<RdMileagepile> mileagepiles = mileagepileSelector.loadByLinkPid(this.getCommand().getLinkPid(), true);
    ...
    public String run() throws Exception {
        ...
        if (!this.getCommand().isCheckInfect()) {
            ...
            log.info("MAIN RDLINK BREAK USE TIME  " + String.valueOf(mainRDLinkTime));
            ...
            if (!this.getCommand().getOperType().equals(OperType.DELETE) && !this.getCommand().getObjType().equals(ObjType.RDBRANCH) &&
                    !this.getCommand().getObjType().equals(ObjType.RDELECEYEPAIR) && !this.getCommand().getObjType().equals(ObjType
                    .LUFACE) && !this.getCommand().getObjType().equals(ObjType.LCFACE)) {
    ...
    private void updataRelationObj() throws Exception {
        ...
        // 交限
        opRefRelationObj.handleRdRestriction(this.getResult(), this.rdLinkBreakpoint, this.getCommand().getNewLinks());

        long refRestrictTime = System.currentTimeMillis();
        log.info(" 交限 USE TIME  " + String.valueOf(refRestrictTime - startRelationObjTime));
        // 分歧
        opRefRelationObj.handleRdBranch(this.getResult(), this.rdLinkBreakpoint, this.getCommand().getNewLinks());
        long refBranchTime = System.currentTimeMillis();
        log.info(" 分歧 USE TIME  " + String.valueOf(refBranchTime - refRestrictTime));
        // 车信
        opRefRelationObj.handleRdLaneconnexity(this.getResult(), this.rdLinkBreakpoint, this.getCommand().getNewLinks());
        ...
        opRefRdElectroniceye.run(this.getResult(), oldLinkPid, this.getCommand().getNewLinks());
        ...
        opRefRdGate.run(this.getResult(), oldLinkPid, this.getCommand().getNewLinks());
        ...
        opRefRdSe.run(this.getResult(), oldLinkPid, this.getCommand().getNewLinks());
        ...
        opRefRdSpeedbum.run(this.getResult(), oldLinkPid, this.getCommand().getNewLinks());
        ...
        opRefRdTollgate.run(this.getResult(), oldLinkPid, this.getCommand().getNewLinks());
        ...
        opRefRdVariableSpeed.run(this.getResult(), this.rdLinkBreakpoint, this.getCommand().getNewLinks());
        ...
        if (!this.getCommand().getOperationType().equals("sameLinkRepair")) {
            ...
            opRefRelationObj.handleSameLink(this.rdLinkBreakpoint, this.getCommand(), this.getResult());
        }
        ...
        opRefRdLane.run(this.getResult(), this.getCommand().getBreakLink(), this.getCommand().getNewLinks());
        long refRdLaneTime = System.currentTimeMillis();
        log.info("详细车道USE TIME    " + String.valueOf(refRdLaneTime - refRdCrossTime));
        // 维护详细车道信息
        OpRefRdMileagepile opRefRdMileagepile = new OpRefRdMileagepile(this.getConn());
        opRefRdMileagepile.run(this.getResult(), oldLinkPid, this.getCommand().getNewLinks());
        ...
        opRefRdTmcLocationLink.run(this.getResult(), this.getCommand().getBreakLink(), this.getCommand().getNewLinks());
