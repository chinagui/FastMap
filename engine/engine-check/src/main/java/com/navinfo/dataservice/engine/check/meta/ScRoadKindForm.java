...
public class ScRoadKindForm{
	...
	public ScRoadKindForm() {
		// TODO Auto-generated constructor stub
	}
	
	public static Map<Integer,Map<String,List<Integer>>> kindFormLoader() throws Exception{
		if(!formMap.isEmpty()){return formMap;}
		synchronized(ScRoadKindForm.class){
			if(!formMap.isEmpty()){return formMap;}
				...
				formMap=runner.query(conn, sql, new ResultSetHandler<Map<Integer,Map<String,List<Integer>>>>() {
					...
					public Map<Integer,Map<String,List<Integer>>> handle(ResultSet rs) throws SQLException {
						while (rs.next()) {
							...
							formMap.put(kind, formMapElement);
							}
						return formMap;
						}
					});	
