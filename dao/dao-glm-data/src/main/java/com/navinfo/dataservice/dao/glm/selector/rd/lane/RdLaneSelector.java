...
public class RdLaneSelector extends AbstractSelector {
	 ...
	 * ResultSet resultSet = null;
	 * 
	 * try { String ids = org.apache.commons.lang.StringUtils .join(linkPids,
	 * ","); String sql = "SELECT lane_pid FROM rd_lane WHERE link_pid in (" +
	 * ids + ") and  u_record !=2 "; if (laneDir != 0) { sql +=
	 * " and lane_dir = :1 "; } sql += " order by link_pid"; if (isLock) { sql
	 * += " for update nowait"; } pstmt = conn.prepareStatement(sql); if
	 * (laneDir != 0) { pstmt.setInt(1, laneDir); }
	 * 
	 * resultSet = pstmt.executeQuery();
	 * 
	 * while (resultSet.next()) { RdLane slope = (RdLane) this.loadById(
	 * resultSet.getInt("lane_pid"), false); lanes.add(slope); }
	 * 
	 * return lanes; } catch (Exception e) { throw e; } finally {
	 * DbUtils.closeQuietly(resultSet); DbUtils.closeQuietly(pstmt); } }
	 */
