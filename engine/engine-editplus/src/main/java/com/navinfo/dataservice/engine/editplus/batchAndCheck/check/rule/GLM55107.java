 ...
 *  检查条件：
	非删除点门牌对象
	检查原则：
	点门牌引导非删除link的属性为高架RD_LINK.IS_VIADUCT=1，报LOG：点门牌引导link为高架路！
 * @author:Jarvis 
 * @date: 2017年10月10日
 */
public class GLM55107 extends BasicCheckRule {
	...
	public void run() throws Exception {
		for(Map.Entry<Long, BasicObj> entry : getRowList().entrySet()){
			...
			if(!obj.getMainrow().getOpType().equals(OperationType.PRE_DELETED) && obj.objName().equals(ObjectName.IX_POINTADDRESS)){
				...
				Integer isViaduct = getIsViaductByLinkPid(linkPid);
				if(isViaduct == 1){
