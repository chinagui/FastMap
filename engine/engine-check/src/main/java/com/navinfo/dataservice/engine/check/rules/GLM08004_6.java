...
/** 
 * @ClassName: GLM08004_6
 * @author songdongyan
 * @date 2016年12月28日
 * @Description: 卡车专用道（加了永久穿行限制，并且车辆类型限制同时且只制作了允许配送卡车、运输卡车、急救车和行人的link）不能作为交限（包括路口和线线结构里的所有交限）的进入线或者退出线（也查线线经过线，但不查线线经过线为交叉口内link的情况）
 * 新增交限/卡车交限：RdRestriction
 * 修改交限/卡车交限：RdRestrictionDetail(新增，修改outLinkPid),RdRestrictionVia(新增，修改LinkPid)
 * 限制类型编辑
 * 车辆类型编辑
 */
public class GLM08004_6  extends baseRule{
	...
	private void checkRdRestrictionVia(RdRestrictionVia rdRestrictionVia, CheckCommand checkCommand) throws Exception {
		...
		if(linkPid!=0){
			...
			log.info("RdRestrictionVia前检查GLM08004_6:" + sql2);
	...
	private void checkRdRestrictionDetail(RdRestrictionDetail rdRestrictionDetail, CheckCommand checkCommand) throws Exception {
		...
		if(!linkPids.isEmpty()){
			...
			log.info("RdRestrictionDetail前检查GLM08004_6:" + sql);
	...
	private void checkRdLinkLimit(RdLinkLimit rdLinkLimit) throws Exception {
		...
		if(rdLinkLimit.status().equals(ObjStatus.INSERT)){
			...
			if((type==2)&&(vehicle==2147483790L)&&(timeDomain==null)){
				checkFlg = true;
			}
		}
		...
		if(checkFlg){
			...
			String sql2 = sb2.toString();
			log.info("RdLinkLimit后检查GLM08004_6:" + sql2);
