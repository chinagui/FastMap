...
public class MapDataPidManager {
	...
	public void recyclePid(List<PidResponse.PidSeg> pidSegs, String clientId) throws SQLException {
		...
		try {
			...
			runner.update(conn,
					"INSERT INTO PID_APPLY_RECORD" + " (RECORD_ID, CLIENT_ID, APPLY_TIME, LIMIT, TABLE_NAME,RECYCLE)"
							+ "    VALUES(?,?,sysdate,0,?,1) ",
					uuid, clientId, "not defined");
	 ...
	 * @return
	 */
	public List<PidResponse.PidSeg> applyPid(Connection conn, String clientId, String tableName, int limit)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	public List<PidResponse.PidSeg> pidDistribute2fastMap(String clientId, String tableName, int limit)
			throws Exception {
		PidDistributeResponse response = PidDistributeResponse.getDefaultResponse();
		List<PidResponse.PidSeg> allPidSegs = new ArrayList<PidResponse.PidSeg>();
		Connection conn = null;
	...
	public String saveDistributeHistory(Connection conn, PidDistributeResponse response) throws SQLException {
		...
		query.update(conn,
				"insert into pid_distribute_record(distribute_id,create_time,client_id,limit,remark,xml)values(?,to_date(?,'yyyy-MM-dd HH24:mi:ss'),?,?,?,?)",
				response.getDistributeId(), response.getCreateTime(), response.getClientId(), response.getLimit(),
				response.getRemark(), xml);
		return xml;
	}
	...
	public String getDistributeXml(String distributeId) throws SQLException {
		String xml = null;
		QueryRunner query = new QueryRunner();
		xml = query.query(dmsDataSource, "select * from pid_distribute_record t where t.distribute_id=?", 1,
	...
	public Page pidDistributeList(int currentPageNum) throws Exception {
		...
		query.query(currentPageNum, 20, dmsDataSource,
				"select distribute_id,create_time,client_id,limit,remark from pid_distribute_record order by create_time desc",
	...
	public void pidImport(String distributeId, String xml, final List<String> returnPidRepeatList) throws Exception {
		...
		// 检查分配批次是否已存在
		int exists = query.queryForInt(dmsDataSource,
				"select count(1) from pid_distribute_record where distribute_id=?", distributeId);
		if (exists > 0) {
		...
		try {
			...
			for (final PidSeg pid : pids) {
				query.query(conn,
						"select t.table_name,t.start_num,t.end_num from pid_segment_usage t "
								+ "      where t.status=1 and t.table_name=? and ((t.start_num between ? and ? ) or (t.end_num between ? and ?))",
						10, new ResultSetHandler() {

							public Object handle(ResultSet rs) throws SQLException {
								while (rs.next()) {
									String msg = "导入号段： " + pid.getPidType() + ":" + pid.getStartNum() + "->"
											+ pid.getEndNum();
									msg += "，库中的号段： " + rs.getString("table_name") + ":" + rs.getString("start_num")
											+ "->" + rs.getString("end_num");
			...
			query.update(conn,
					"insert into pid_distribute_record(distribute_id,create_time,client_id,limit,remark,xml)values(?,to_date(?,'yyyy-MM-dd HH24:mi:ss'),?,?,?,?)",
					pidDistributeResponse.getDistributeId(), pidDistributeResponse.getCreateTime(),
					pidDistributeResponse.getClientId(), pidDistributeResponse.getLimit(),
					pidDistributeResponse.getRemark(), xml);
			...
			for (final PidSeg pid : pids) {
				...
				query.update(conn,
						"insert into pid_apply_record(record_id,client_id,apply_time,limit,table_name,recycle)"
								+ "values(?,?,?,?,?,?)",
						record_id, "PID导入", new java.sql.Timestamp(new Date().getTime()),
						(pid.getEndNum() - pid.getStartNum() + 1), pid.getPidType(), "1");
				// 写入号段表
				query.update(conn,
						"insert into pid_segment_usage(usage_id,record_id,table_name,start_num,end_num,status)"
								+ "values(?,?,?,?,?,?)",
						usage_id, record_id, pid.getPidType(), pid.getStartNum(), pid.getEndNum(), 0);
				// 更新表最大值
				query.update(conn,
						"update pid_max_per_table t set pid_max_value=?  where t.table_name=? and t.pid_max_value<?",
	...
	public List<PidSeg> applyPid(String clientId, String tableName, Integer limit, String clientIp, String clientTaskId)
			throws Exception {
		List<PidResponse.PidSeg> allPidSegs = new ArrayList<PidResponse.PidSeg>();
		Connection conn = null;
		try {
			...
			if (StringUtils.isBlank(tableName)) {
				...
				for (int i = 0; i < tables.size(); i++) {
					tableName = tables.get(i);
					List<PidResponse.PidSeg> pidSegs = applyPid(conn, clientId, tableName, limit, clientIp,
							clientTaskId);
					allPidSegs.addAll(pidSegs);
				}
			} else {
